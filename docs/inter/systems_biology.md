...
“系统”可以是一条由少数分子组成的反馈回路，也可以是代谢网络、细胞群体、发育组织或器官间交换。系统生物学由研究方式界定：先明确边界和观测量，再把相互作用组织成模型，用模型提出定量预测，最后通过新的测量与扰动检验并修正解释。网络结构、确定性动力学、随机与空间过程以及模型推断组成一条从关系图到可检验机制的学习路线。[^systems-textbook]

## 系统的边界、组分、关系与环境 { #system-definition }
...
## 状态、输入与输出 { #states-inputs-outputs }

状态变量描述在给定时刻足以继续预测系统演化的信息。它可以是分子数、浓度、修饰比例、膜电位、细胞位置、群体中不同状态的比例，或经过合理粗粒化的有效变量。参数则描述反应速率、结合强度、扩散系数或调控增益等相对稳定的关系。两者的区别依模型尺度而定：细胞内三磷酸腺苷（adenosine triphosphate，ATP）浓度在秒级信号模型中可能作为固定参数，在代谢模型中却是随时间变化的状态。
...
## 结构、动力学与功能 { #structure-dynamics-function }
...
## 跨尺度连接与涌现观测量 { #scales-emergence }

生命系统跨越分子、细胞、组织、个体和群体尺度。下层过程为上层行为提供机制，上层环境又改变下层过程的工作条件。受体结合改变单细胞信号，许多细胞之间的通信形成同步或空间边界；组织灌流决定氧和底物供应，细胞代谢反过来改变局部耗氧和分泌。跨尺度模型需要明确接口变量及其单位，使不同层级通过可测量关系衔接。
...
## 测量策略与模型可见范围 { #measurement-strategy }

组学测量扩大了可同时观察的组分范围。转录组记录核糖核酸（ribonucleic acid，RNA）丰度，蛋白质组和修饰组接近蛋白容量与状态，代谢组记录代谢物池，互作与染色质实验提供候选关系。它们落在不同物理层和时间尺度：转录本丰度、蛋白活性、代谢物浓度和通量是不同的观测量。多组学整合要保留每一层的观测过程，再通过共享反应、调控关系或时间顺序建立联系。[^systems-practice]
...
## 模型—实验循环 { #model-experiment-cycle }
...
1. 界定系统边界，确定状态变量、输入、输出和可测量的观测量。
2. 提出结构、动力学或约束模型，写明其中保留的机制假设。
3. 用已有数据估计参数并产生定量预测，同时标出预测的不确定性。
4. 选择能让候选机制产生不同结果的扰动、剂量和采样时点。
5. 获得新的时间、空间或单细胞数据，比较预测与观测。
6. 若预测得到支持，就在新的条件下继续检验；若出现系统偏差，就修正边界、结构、参数或观测模型，再形成下一轮预测。
...
这些专题构成相互衔接的分析路径。网络结构给出变量和关系，动力学说明这些关系怎样随时间运行，随机与空间模型补入离散事件和多细胞耦合，推断与实验设计再判断哪些结构和参数真正受到数据支持。学习时可沿同一个例子往返各主题：先画出调控网络，再写出状态与反馈，随后考虑噪声和空间，最后设计能够区分候选机制的实验。

## 相邻学科接口 { #disciplinary-interfaces }

系统生物学与生物信息学都处理网络和组学数据，关注点有所分工。生物信息学说明数据库证据、网络构建、矩阵分析和多组学数据流程；系统生物学从这些结果出发，研究系统行为、机制模型和扰动预测。统计学提供估计、不确定性、模型诊断与因果推断的通用语言，系统生物学把这些方法放进具体的生物反应、状态和实验设计中。
...
系统生物学最终追求的是可检验的因果解释：说明哪些组分在什么边界和条件下相互作用，怎样产生可观察的时间与空间行为，以及改变某个环节后系统将如何响应。完整的组分清单、精确的模型和高通量数据都服务于这一目标；它们在测量—模型—扰动—检验的循环中相互校正，才逐步形成可靠的系统知识。
