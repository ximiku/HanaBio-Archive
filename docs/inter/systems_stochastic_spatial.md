...
确定性方程追踪浓度如何随时间变化，默认相同初始状态在相同参数下沿同一条轨迹前进。细胞内的反应却由一次次分子碰撞、结合、转录和降解事件组成；同一时刻，两个遗传背景相同、处于相同培养条件的细胞也可能含有不同数目的信使核糖核酸（messenger RNA，mRNA），经历不同的启动时刻，随后落入不同状态。空间又使这种差异能够传播：分子扩散形成梯度，邻近细胞交换信号，局部回路通过组织尺度的耦合产生条纹、行波或同步节律。

系统的状态描述依次从连续浓度转为离散分子数和概率分布，从充分混合的反应器扩展为空间场，最后连接为由移动、分裂和通信的细胞组成的群体。确定性反馈、双稳态和极限环见[反馈、开关与振荡](systems_dynamics.md)，扩散的物理起源与通量定律见[随机运动、扩散与膜输运](biophysics_transport.md)；这里关注随机性与空间耦合怎样改变系统行为。[^systems-textbooks]

## 反应事件与整数随机状态 { #discrete-reaction-events }
...
其中 $\mathbf n=(n_1,n_2,\ldots)$ 是各分子的拷贝数，$\nu_j$ 记录第 $j$ 条反应发生一次时每种分子增加或减少多少。给定当前状态，在很短的时间 $\Delta t$ 内发生这条反应的概率近似为 $a_j(\mathbf n)\Delta t$；$a_j$ 称为倾向函数，它把反应速率常数与当前可用的反应物数目结合起来。一次降解使拷贝数减一，一次转录起始产生一条核糖核酸（ribonucleic acid，RNA），一次细胞分裂还会把分子随机分配给两个子细胞。

当分子很多、单次事件只造成很小的相对变化时，许多随机跳跃的平均趋势可由连续浓度和速率方程近似。低拷贝状态缺少这种平滑性：一个只有两份的转录因子增加一份就是 50% 的变化，而一万份代谢物增加一份几乎不可见。体系体积同样重要；相同浓度放进更小的区室，对应的分子数更少，相对涨落通常更大。脱氧核糖核酸（deoxyribonucleic acid，DNA）位点、活跃启动子、少数调控 RNA 和局部信号复合物因此常需要离散描述。

随机反应仍受倾向函数、网络拓扑、降解和反馈共同制约：倾向函数规定各种事件的相对机会，网络拓扑规定每次跳跃改变哪些变量，降解和反馈则决定涨落保留多久、向哪里传播。随机模型描述轨迹集合的分布、相关性、首次越阈时间和长期状态占比。

## 化学主方程的状态间概率流 { #chemical-master-equation }
...
Fano 因子 $F=\operatorname{Var}(n)/\langle n\rangle$ 在这个基准中等于 1，变异系数（coefficient of variation，CV）$\mathrm{CV}=\sqrt{\operatorname{Var}(n)}/\langle n\rangle$ 则随平均数按 $1/\sqrt{\langle n\rangle}$ 下降。启动子切换、成串转录、反馈、细胞周期和共享环境会使实际分布偏离这一基准；观察到 $F>1$ 提示存在额外涨落来源，具体机制仍需进一步检验。

## Gillespie 模拟的反应与等待时间 { #stochastic-simulation }
...
算法中的“精确”有明确边界：它指在给定反应网络、倾向函数和马尔可夫（Markov）假设下，直接抽取事件时刻而不采用有限时间步近似。反应机制、速率常数和充分混合假设仍是需要检验的模型前提。一条模拟轨迹只表示一次实现；均值、方差、状态占比、首次到达时间和罕见切换率都需要重复轨迹或等价的概率计算，并与单细胞数据的观测过程一起比较。

分子数增大、分布变窄时，随机轨迹的相对波动减小，确定性模型成为合适近似。网络同时含有快而高拷贝的代谢反应、慢而低拷贝的转录事件时，可按时间尺度和拷贝数采用混合模型。问题中的观测量与可接受的近似误差决定采用连续、离散还是混合描述。

## 内禀与外禀噪声的系统边界 { #intrinsic-extrinsic-noise }
...
## 转录爆发、非线性回路与分布形状 { #bursting-switching }
...
## 随机游走、扩散与空间通量 { #diffusion-reaction-diffusion }
...
## 源、扩散、清除与位置坐标 { #morphogen-gradients }
...
## Turing 不稳定性与空间波长 { #turing-instability }
...
盘基网柄菌在饥饿后通过细胞外环磷酸腺苷（cyclic adenosine monophosphate，cAMP）脉冲协调聚集，是从随机单细胞事件到群体波的经典实例。活细胞成像和灌流实验显示，低于群体阈值时个别细胞随机发放脉冲；细胞密度与细胞外 cAMP 累积提高后，群体转入规则节律，较高频的信号中心再锁定周围细胞并形成持续传播。[^gregor-collective] 这说明噪声可以提供启动事件，正反馈式接力放大事件，群体耦合最后选择并稳定宏观节律。

## 细胞耦合、相位噪声与群体同步 { #coupled-oscillators }
...
斑马鱼体节钟提供了在体证据。Horikawa 等结合嵌合实验和数学模拟表明，受细胞内 Her/Hairy 振荡调制的 Notch 细胞间通信会耦合相邻振荡器；随机基因表达和细胞增殖造成相位扰动，耦合则减小这些扰动并维持整体同步。[^horikawa-coupling] 体节形成中的时钟—波前与组织轴关系见[脊椎动物体节时钟](../development/axis_patterning.md#vertebrate-somitogenesis)；这里关注多个噪声振荡器如何形成群体相位场。

## 状态转换、增殖、通信与群体组成 { #population-states }
...
## 噪声、传播与自组织的多尺度实验 { #multiscale-experiments }
...
从随机事件到群体状态的各层模型可以相互嵌套，但各自的参数对应不同尺度。分子倾向函数、细胞状态转移率、有效扩散系数和群体耦合强度来自不同尺度的粗粒化；上一层被省略的变量会在下一层表现为记忆、相关噪声或条件依赖参数。模型选择、参数估计、可辨识性和新扰动检验见[系统建模、参数推断与实验检验](systems_modeling_inference.md)。人工设计的通信线路和空间图案归入[多细胞合成系统与空间模式](synthetic_multicellular.md)，这里讨论自然系统的解释性模型。
