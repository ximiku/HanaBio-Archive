...
土壤与潮间带动物调查涉及分类、计数、称量、优势种判定和多样性计算；下文还完整重算六个样方的最近邻聚类练习。取样框、潮位或土层记录见[环境梯度与生态生理实验](ecophysiology.md#soil-intertidal-sampling)，多样性与群落结构的生态学含义见[群落组成、结构与多样性](../../ecology/community_ecology.md#alpha-diversity)，多元方法的一般原理见[多元统计、降维与分类](../../biostat/multivariate_models.md#distance-and-ordination)。
...
海葵、海星、海胆、蛏、海岸水虱、牡蛎和弹涂鱼来自不同潮间带微生境，应与农田或凋落物中的土壤动物分别界定样本群落。每批样本应先确定纳入对象和最低分类阶元，再用检索表、连续形态特征和必要的专家复核鉴定。物种名尚不能确定时，可使用稳定的“形态种 A”“多毛类 B”等操作分类单元，并保留现场照片、样品编号、采集地点、日期、鉴定者和置信度；后续改名时更新分类映射，而不改写原始计数。
...
$y_i$ 可以是数量、盖度或生物量，但一项分析中只能采用一种明确定义的量。数量优势种未必具有最高生物量，盖度最高的固着种也未必个体最多；“个体数、盖度、生物量”应分别排序，三个单位不同的指标不宜直接相加。植物群落常用的相对密度、相对频度和相对优势度综合重要值有特定定义，应用于动物样本时须另行说明；其计算见[群落组成、结构与多样性](../../ecology/community_ecology.md#importance-value)。
...
六样方练习对样方 $j$、$k$ 使用欧氏距离（Euclidean distance）：
...
| 赫林格（Hellinger）变换后欧氏距离 | 先令 $h_{ji}=\sqrt{x_{ji}/\sum_i x_{ji}}$ | 比较相对组成，并减弱优势种和许多零值对欧氏几何的扭曲 |
...
样方 3 与 4 的距离应为 $\sqrt{51}=7.141$。随表给出的距离矩阵却分别记为 4.472 和 1.732。检查全部 15 个两两距离可发现：只要把 $x_{3c}$ 从 7 改为 1，距离矩阵便全部成立；例如
...
“涉及合并样方的数据求平均”描述的是平均连接法（average linkage），而最近邻更新采用单连接法（single linkage）。平均连接按两个簇中所有跨簇样本对的距离求平均，单连接则取最小成对距离；两种规则会产生不同树。SciPy 的正式方法说明还指出，最小距离并列时，不同实现可能选取不同的二叉合并次序。[^linkage-methods]
...
若 $x_{3c}=7$ 确为原始观察，随表给出的距离矩阵便应废弃并从丰度表重算。最近邻结果改为：样方 4 与 5 在 2.236 合并，样方 1 与 6 在 2.449 合并；树高 3.000 处样方 2 和 $\{4,5\}$ 都通过与 $\{1,6\}$ 的最短距离接入；样方 3 最后才在 5.196 接入。一个单元格即可改变聚类结果，因此数据审计是解释树状图的必要环节。

![原表与候选修订产生的两棵单连接树](../_img/群落多样性与分类分析_01.svg){ loading=lazy }
/// caption
同一组数据中，$x_{3c}=1$ 可复现随表给出的距离矩阵，$x_{3c}=7$ 则使样方 3 成为最后接入的离群样方；横线高度为欧氏距离。
///

树状图底端是原始样方，向上逐步形成较大的簇，顶端是包含全部样方的根簇；纵轴是本例的欧氏距离，而非相似度。单连接只需一对近邻就能把两个簇接在一起，容易产生细长的“链化”分组。所谓“空间压缩”是指这种最小距离更新对簇内远端差异不敏感，并不表示原始样方在地理空间中被压缩。
...
聚类属于探索性分类。可比较原始欧氏距离、Hellinger–Euclidean 与 Bray–Curtis，并比较单连接、平均连接或其他有明确几何条件的方法；若分组随一个单元格、一次变换或一种连接法彻底改变，结论应报告为不稳定。预先记录的土层、潮位、基质和植被可在聚类完成后叠加解释，但不能先按环境标签修改样本值，再声称聚类独立发现了这些组。
