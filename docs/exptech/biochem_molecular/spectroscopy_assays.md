...
不同检测模式观察的是不同物理过程。它们可以共享一台多模式读板机，各自的解释规则则需要针对相应信号验证。
...
定性显色首先回答“在规定条件下是否出现相容的化学反应”。还原糖使斐林或班氏体系中的铜发生还原并形成沉淀，淀粉与碘形成有色复合物，双缩脲反应报告足够数量的肽键；Millon 反应主要响应蛋白质中的酪氨酸酚基，但传统试剂含汞和强酸，只能在严格化学安全条件下作为方法史与特定化学反应讨论。苏丹 III／IV 等脂溶性染料进入疏水相，间苯三酚—盐酸和番红可用于木质化组织观察。甲基绿—派罗宁、二苯胺等核酸染色还受到聚合状态、固定、pH 和其他组分影响。黄色碘液不构成蛋白质阳性反应，而其他可发生同类反应的物质则需由选择性对照区分。经典反应可作为化学性质和标本观察的学习入口；升级为定量方法还需建立选择性、校准和基质证据。
...
表面等离子共振（surface plasmon resonance，SPR）利用界面共振测量分子结合，原理不同于普通溶液吸收测定。偏振光经棱镜照射金属薄膜，在特定角度或波长把能量耦合到金属—介质界面的表面等离子模式；靠近表面的折射率变化会移动共振条件。将一种结合分子固定在传感表面，让另一种分子随流动相通过，仪器便可实时记录结合与解离造成的响应变化。分析物无需荧光或放射性标签，但固定化、表面取向和非特异吸附仍可能扰动体系。
