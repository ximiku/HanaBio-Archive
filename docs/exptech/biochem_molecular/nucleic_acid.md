...
原素材中的 TE 为 10 mmol/L Tris–HCl（pH 8.0）与 1 mmol/L EDTA。Tris 提供近中性的缓冲环境，EDTA 有助于抑制金属依赖核酸酶；若核酸即将进入依赖 Mg$^{2+}$ 的低体积反应，EDTA 的带入也须计入。水、低 EDTA 缓冲液或其他洗脱液各有保存与下游兼容性边界。部分离心柱需要使用前平衡或预润湿，是否需要由具体膜和说明书决定，不是所有硅胶膜共有的“活化”步骤。
...
原课堂记录使用下列小量柱式流程。它保存了操作顺序，但 `Buffer 1/2/3`、`DW1`、`Wash Solution` 和柱膜都属于特定试剂盒体系；离心速度、培养体积和洗脱量不能脱离说明书直接移植。

该记录把 Buffer 1 写为 50 mmol/L 葡萄糖、10 mmol/L EDTA、25 mmol/L Tris–HCl（pH 8.0），Buffer 2 写为 0.2 mol/L NaOH 和 1% SDS，Buffer 3 写为 2 mol/L 醋酸／3 mol/L 醋酸钾（pH 4.8）。这些浓度保存了经典葡萄糖—Tris—EDTA、碱—SDS 与醋酸钾中和体系，但 Buffer 3 的配制表述、各液最终 pH 和试剂盒兼容性仍须按实际配方核对。
...
原理笔记还保留了酚／氯仿／异戊醇 25∶24∶1 的经典抽提液：酚使蛋白变性，氯仿改善分相并减少水相中的酚，少量异戊醇可降低起泡和界面乳化。具体比例、酚的 pH 与抗氧化剂取决于目标是 DNA 还是 RNA；有机抽提也不能替代对残余酚和下游抑制的检查。

原课堂植物流程以 0.15 g 叶片、600 μL `PCL Buffer`、12 μL β-巯基乙醇和少量 SiO$_2$ 研磨，取 400 μL 匀浆在 65 ℃ 放置 45 min，其间约每 10 min 温和混匀；随后加入 200 μL `Buffer PP`，在 −20 ℃ 放置 5 min，离心后取上清，以 500 μL 氯仿分相。约 300 μL 水相再用等体积异丙醇或 2.5 倍体积乙醇沉淀，75% 乙醇洗涤，短时晾干后用 70 μL TE 溶解。这个版本完整保留“研磨—裂解—除杂—分相—沉淀—洗涤—复溶”的顺序；其中商业缓冲液成分、离心转子的 RCF、氯仿稳定剂与通风条件未给出，不能作为可直接复现实验的独立配方。
...
原课堂柱法先向 `AC` 柱加入 500 μL 平衡液，并以 13,000 r/min 离心 1 min；随后从 30 mg 新鲜组织开始，加入 200 μL `TL` 和 20 μL 蛋白酶 K，55 ℃ 消化约 30 min；再加 200 μL `CB`，70 ℃ 约 10 min，冷却后加 100 μL 异丙醇。去除大块不溶物后上柱，以 `IR` 500 μL 和 `WB` 500 μL 依次洗涤，空柱离心，再向膜中央加入 100 μL、约 65 ℃ 的 `EB`，静置 5 min 后洗脱。原记录只给 rpm 而没有转子半径，无法换算 RCF；温度、缓冲液缩写和柱容量也都是试剂盒专属条件，报告时必须补齐厂家、版本和偏离说明。动物来源、病原风险和处置要求见[实验安全、伦理与风险管理](../foundations/safety_ethics.md#animal-and-field-ethics)。
...
原课堂记录包含玻璃器皿 180 ℃ 干烤 6 h、塑料器皿以 0.1% DEPC 水浸泡 12 h、有机玻璃电泳槽用去污剂—水—乙醇—3% H$_2$O$_2$—DEPC 水处理的路线。这些做法保留了早期 RNA 实验对去 RNase 的重视，却不能成为所有材料的固定 SOP：高温和氧化剂会损伤部分塑料与设备，DEPC 会与胺类缓冲成分反应，而且浓缩试剂对皮肤、眼和呼吸道有刺激危害。现在通常优先采用认证的 RNase-free 一次性耗材、专用工作区和与设备兼容的商业去污剂；确需 DEPC 时须按 SDS、材料兼容性和机构 SOP 配制、分解与处置。[^rnase-control-source]
...
原课堂小鼠肝流程以 900 μL 单相试剂裂解组织，室温放置约 5 min，约 12,000 ×g、4 ℃ 离心；加入 200 μL 氯仿并充分乳化，静置后再次离心，取上层水相约 400 μL，以等体积异丙醇沉淀，乙醇洗涤，短时干燥后用 25 μL RNase-free 水溶解。它完整保留“均质—分相—取水相—沉淀—洗涤—复溶”的主线。氯仿具有挥发和暴露风险，胍盐废液不得与含次氯酸盐的废物流任意混合；实际体积、温度和时间必须以所用试剂说明书、样本量和机构化学卫生要求为准。
...
原素材把 cDNA 文库、Northern 和 RT-PCR 按“完整性要求高低”排成固定等级。实际敏感性由文库构建方式、目标长度、引物位置、是否需要全长信息和允许的降解背景决定：短 RT-qPCR 扩增子可能在部分降解样本中仍能产生信号，却可能出现 3′／5′ 偏倚；Northern 需要目标区域和迁移长度可解释；全长转录本和异构体分析对长分子连续性更敏感。质量阈值应由预定用途和验证实验决定，而不是由技术名称自动决定。
...
$A_{260}/A_{280}$ 约 1.8（DNA）或约 2.0（RNA）、$A_{260}/A_{230}$ 约 2 左右常用作经验参考。比值会受 pH、离子强度、空白、仪器光程和样品浓度影响；低 260/230 可见于酚、胍盐、糖类、EDTA 或其他提取成分，低 260/280 可提示蛋白或酚，但任何单一比值都不能诊断唯一污染物。约 320–340 nm 的基线更适合发现浑浊、气泡和散射，原素材把 `OD310` 偏高直接解释为盐污染并无普遍依据。微量吸光和目标选择性荧光的互补边界亦见仪器技术资料。[^spectral-fluorescence-qc]
...
提高琼脂糖浓度通常缩小有效孔径，改善较短片段的分辨；降低浓度有利于较大片段进入和展开，但凝胶更脆。原素材把 0.70%、1.0%、1.40% 和 2.0% 胶列为由大片段到小片段的教学梯度，这一方向应保留，具体分辨范围须由琼脂糖类型、缓冲液、场强和 ladder 实测，不能给每个浓度配一个跨实验室固定截止值。
...
上样液中的甘油、蔗糖或其他密度成分使样品沉入孔内，溴酚蓝和二甲苯青等示踪染料显示前沿，不代表 DNA 本身。原素材记录的近似位置如下；因未注明缓冲液、场强、染料配方和 DNA ladder，只能用来理解“示踪染料速度随胶浓度改变”，不能据此精确判断目标片段已经跑到哪里。
...
| --- | ---: | ---: | ---: | ---: |
| 溴酚蓝的原记录等效位置 | 约 1 kb | 约 0.6 kb | 约 0.2 kb | 约 0.15 kb |
| 二甲苯青的原记录等效位置 | 未记 | 约 2 kb | 约 1.6 kb | 未记 |

### 制胶、上样与运行 { #gel-casting-running }

原课堂实验以 0.3 g 琼脂糖加入 30 mL TBE，得到名义 1% 凝胶；微波加热至完全透明，冷却到约 60 ℃ 后加入 3 μL `EB`，插梳并灌胶，凝固后让电泳液刚好覆盖胶面。这里 `EB` 推测为 ethidium bromide，但原记录没有储液浓度，因而这 3 μL 不能用于重建安全浓度。实际操作应使用经验证的染料配方，按其危害、成像光源、废物路线和是否影响迁移选择胶内染色或后染。
...
原步骤把 2 μL 6× loading buffer 滴在手套上再与 10 μL DNA 混合，并要求上样末端打出半个气泡。这两项都会增加污染或把样品推出孔外，应改为在洁净管或封口膜上混匀，枪头进入缓冲液并停在孔口上方，缓慢排尽液体但不排出空气。泳道位置和 ladder 必须预先记录；边缘泳道是否可用取决于胶盘、温度和设备，不能一律空置。过量 DNA、盐或颗粒会造成拖尾、孔内滞留和局部导电异常。
