...
系统综述先用可重复的方法界定问题、检索和筛选研究，再评价每项证据的设计与偏倚。荟萃分析是在研究具有共同科学含义时进行的定量综合步骤；它把研究内不确定性和研究间差异放进同一模型，却不能替代系统综述的检索、选择与评价过程。

进入综合的效应估计来自不同研究设计。若各研究所对应的总体、处理、时间零点、结局或待估量不同，一个共同效应尺度仍不足以保证它们可以合并；若单项研究只支持关联，合并更多关联也不会自动产生因果解释。关于单项研究怎样界定并识别干预效应，见[因果推断](causal_meta_analysis.md#association-to-effect)。
...
一份可复核的系统综述应给出方案与偏离、完整检索、筛选流程、数据和代码、逐研究效应、偏倚评价、合并模型、\(\tau^2\)、\(I^2\)、预测区间、影响分析、缺失证据诊断和证据确定性。若研究不能合并，应说明阻止合并的科学或方法学差异，并用透明的结构呈现各项证据，而不是把“未做荟萃分析”视为综述失败。
