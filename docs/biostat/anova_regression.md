...
方差分析（analysis of variance，ANOVA）处理的核心问题是：当若干组观测均围绕各自的总体均值波动时，组间均值的差异能否由实验误差解释。它先将全部观测的变异按模型来源分解，再比较某一来源的均方与合适的误差均方。单因素 ANOVA、析因 ANOVA、区组分析和协方差分析看似各有一套表格，实质上都是一般线性模型对不同设计矩阵与假设的表达。

平方和、自由度与 F 表揭示了变异怎样被分配，也能暴露原始数据中的实验单位、平衡性和误差层级。设计矩阵则把这些经典手算原理扩展到不等样本量、交互作用、连续协变量和更复杂的效应结构。两条路径共同指向同一个要求：检验对象、误差项和可推广范围都必须由研究设计决定。

## 试验语言与模型对象 { #design-language }
...
试验单位是能够独立接受某个处理的最小单位。把一个处理施加到多个试验单位，才形成生物学意义上的重复。若每只小鼠接受一种饲料，而每只小鼠取三张切片，处理的重复数是小鼠数，切片则是同一试验单位内的子样本。重复测量、技术复孔和空间子样方可以提高测量精度，处理分派的独立信息仍由实验单位提供。实验单位与观测单位的区别已经在[生物数据整理与描述统计](data_description.md#population-to-table)中建立，ANOVA 的误差层级必须继续保留这个结构。

效应是模型中与某一因素或水平组合相联系的响应差异。主效应描述一个因素在指定的其他因素分布上平均后的对比；交互作用描述一个因素的效应随另一因素水平而改变。二者都是相对于明确平均方式和参照水平定义的模型对比。
...
固定与随机效应的分类取决于待估目标、水平来源与推广范围。操作者可以被严格安排，却仍可能代表更广泛的操作者总体；年份无法由研究者制造，却可作为几个特定年份的固定比较。判断时应先问：结论只关于这些水平，还是要推广到未观察水平？模型要估计各水平对比，还是估计水平间方差？

固定效应和随机效应同时存在时形成混合效应模型。例如，三种固定处理在随机抽取的地点中重复，模型既估计处理对比，也估计地点及地点内相关。随机因素会改变期望均方、F 检验分母和自由度，复杂设计需要为每个效应匹配相应误差项。随机、混合与重复测量模型将在[广义线性模型与混合效应模型](generalized_mixed_models.md)中继续展开，这里说明它们与经典 ANOVA 表的接口。
...
备择假设只说明至少一组均值不同，未指出具体组别和差异方向。拒绝总体零假设后仍需用预先规定的对比或适当的多重比较来回答具体问题；未拒绝则表示这组数据对总体差异的证据不足。
...
\(SS_A\) 衡量组均值围绕总均值的变异，\(SS_E\) 衡量同组观测围绕组均值的剩余变异。后者叫残差平方和，其中既可能包含随机测量误差，也可能包含遗漏的批次、时间趋势、异方差或依赖结构。
...
这些公式同时适用于相等和不等组样本量，校正数始终为总和平方除以总样本量，即 \(T^2/N\)。Penn State 对一般线性模型的手算说明也给出这一恒等式及 \(T_i^2/n_i\) 的处理平方和。[^psu-linear-model]
...
总均值的估计消耗一个自由度；\(k\) 个组均值相对于总均值只有 \(k-1\) 个独立偏差；每组估计一个均值后，残差共有 \(N-k\) 个自由度。因此组间、残差与总自由度依次为 \(k-1\)、\(N-k\) 和 \(N-1\)。NIST 的 ANOVA 表给出相同分解。[^nist-anova-table]
...
若组均值确有差异，\(MS_A\) 还包含均值偏离所贡献的变异，F 往往增大。F 检验比较模型解释的变异与残差变异；“处理效应方差”由模型分解得到，并非直接观测量。两组时，等方差单因素 F 检验与合并方差（pooled-variance）*t* 检验满足 \(F=t^2\)，这也说明 ANOVA 是均值线性模型的扩展。
...
编码决定单个系数回答什么问题。参照编码下，截距是参照组均值，其余系数是各组与参照组之差；和为零编码下，截距对应一组规定权重的总体水平，系数表达相对偏离。只要模型张成同一列空间，拟合值和所有可估计均值对比一致。单个虚拟变量系数检验与因素的 \(k-1\) 自由度总体检验对应不同假设。

R 的 `aov` 本身通过 `lm` 拟合 ANOVA，并以传统 ANOVA 语言显示结果；官方文档同时提醒，它主要为平衡设计组织输出，不平衡或多误差层设计更难解释。[^r-aov] 这反映了 ANOVA 与回归共享同一线性模型骨架，差别主要来自预测变量的尺度、设计结构以及要检验的线性函数。这里的**一般线性模型**（general linear model）假定响应由参数的线性组合表示；**广义线性模型**（generalized linear model）则允许非正态响应和链接函数。
...
ANOVA 表应与各组样本量、原始点、均值及区间一起阅读。总体 F 很大可能来自一个组远离其余组，也可能来自沿剂量有序变化的多个均值；具体形状需要由均值模式和预先定义的均值差、趋势或处理组合差及其置信区间说明。
...
样本 \(\omega^2\) 可能略小于零，这表示估计在零附近波动；解释变异在总体意义上仍以零为下界。多因素设计中还常见偏 \(\eta^2\)，其分母只含该效应与误差平方和，跨设计比较时需说明与普通 \(\eta^2\) 的区别。效应量应注明公式并尽可能给出区间；方法学综述也强调 ANOVA 效应量的用途依赖组间、组内设计和后续综合目标。[^anova-effect-size]
...
在平衡设计中，若两组对比系数的内积为零，它们相互正交；最多可以用 \(k-1\) 个正交单自由度对比完全分解 \(SS_A\)。不平衡设计中的正交性要按估计协方差加权。计划对比减少了无目标的搜索，但多个确认性对比仍属于一个检验家族，需要按事前科学主张决定是否及怎样控制多重性。
...
若 \(\lvert\bar Y_{i\cdot}-\bar Y_{j\cdot}\rvert>LSD_{ij}\)，该未调整比较在单次 \(\alpha\) 水平拒绝均值相等。先要求总体 F 显著再做 LSD，称为保护性 LSD（protected LSD）；它在经典路径中为多次 *t* 检验增加总体门槛。然而组数增加时，它仍不能对所有两两比较提供一般的强家族错误率控制。生理学方法综述指出，LSD 与 Newman–Keuls 在许多实验情形下都会失去这种控制。[^multiple-comparisons-review]

LSD 的用途因目标而异。少数事前对比可直接检验并按该小型家族调整；若所有两两差异都属于确认性结论，通常应使用明确控制全家族错误率的方法。总体 F 与预设对比回答不同假设：一个狭窄而预设的对比可能显著，即使 \(k-1\) 自由度的总体检验未拒绝；总体显著也不保证任一经严格调整的两两比较显著。

### 学生化极差与 Tukey 方法

把有序样本均值的极差除以均值标准误，可得到学生化极差（studentized range）统计量 \(q\)。R 的正式分布文档把它定义为正态样本极差 \(R\) 与独立方差估计尺度之比，并由均值个数和误差自由度确定分布。[^r-studentized-range] 对所有 \(k(k-1)/2\) 个两两比较，Tukey 诚实显著差（Tukey honestly significant difference，Tukey HSD）在平衡设计中使用同一个全极差临界值；不等样本量的 Tukey–Kramer 区间可写为
...
这组区间共同具有规定的家族覆盖率。R 的 `TukeyHSD` 文档明确说明，普通逐对 *t* 区间会抬高至少一次错误声明的概率，而 Tukey 区间以学生化极差控制整组两两比较；其不等样本量调整适合轻度不平衡。[^r-tukey]

Tukey–Kramer 尺度为上式中的
\(\sqrt{MS_E(1/n_i+1/n_j)/2}\)。\(\lvert r_i-r_j\rvert+1\) 表示排序后两均值之间跨越的均值个数 \(p\)，误差自由度则来自 ANOVA 模型。
...
一些中文农学教材把 Duncan 新复极差检验使用的临界表称为 SSR，并与 SNK 一起归入最小显著极差法。Duncan 方法又进一步按跨距改变逐步错误率，通常比 SNK 更宽松。这些名称和按秩次距查表的历史计算路线需要区分：\(q\) 是学生化极差统计量，Tukey、SNK 与 Duncan 使用临界范围的方式各不相同。方法应按比较家族、错误率目标、设计平衡性和科学代价选择；拒绝数量本身不表示精度高低。[^mean-comparison-review]
...
字母标记法适合压缩大量两两结果：没有共同字母表示相应比较达到所用方法的阈值，共享字母表示该比较未拒绝。均值等效需要预设界值另行判断，差值大小和区间宽度也需在正式结果中呈现。因此还应给出均值或估计边际均值、差值、同时置信区间、调整方法和完整比较家族。
...
交互作用表示 \(A\) 的某个对比随 \(B\) 的水平改变。例如同一药物在两个基因型中的均值差方向相反，边际平均后可能得到接近零的药物主效应，但两个基因型内仍分别存在方向相反的条件效应。解释时应先画各单元均值与区间，检验交互，再根据科学问题考察简单效应或交互对比。Penn State 的析因模型给出同一模型与自由度分解。[^psu-factorial]

交互未显著表示当前设计对非加性效应的证据不足，较小样本可能缺乏识别交互的能力。若分析计划允许删去交互，应结合区间、功效、机制和层级原则作出决定。保留高阶交互时通常也保留其组成主效应，使模型参数和预测保持层级结构。
...
把区组间变异从残差中分离，可能提高处理对比精度。每格只有一个观测时，处理—区组交互无法与误差分开，模型实际上假定该交互可忽略；随机化也必须在区组内完成。Penn State 对随机完全区组设计（randomized complete block design，RCBD）的说明强调每个处理在每个区组出现一次并在区组内随机分派。[^psu-blocks] 区组是固定还是随机仍由推断目标、水平来源和推广范围决定。
...
其中 \(B(A)\) 表示 \(B\) 嵌套于 \(A\)，其结构不同于 \(A\times B\) 的交互。固定平衡模型下 \(B(A)\) 有 \(a(b-1)\) 个自由度；随机嵌套设计中，检验 \(A\) 常需用 \(MS_{B(A)}\) 而非最底层残差作分母。Penn State 对嵌套随机效应的期望均方展示了这种不同误差项。[^psu-random-effects]

裂区、重复测量和同一培养皿内多个样本进一步产生多个随机化层级。分析应从处理实际施加在哪一级开始构造误差结构，而非仅在普通 ANOVA 表中增加一行因素。
...
共同斜率模型假定协变量与响应的关系在各处理组相同。应先检查包含 \(\tau_i(X_{ij}-X_0)\) 的交互模型；若斜率确实不同，处理差异随 \(X\) 改变，应在有数据支持的协变量范围内报告若干 \(X\) 水平的条件效应。Penn State 的 ANCOVA 步骤也把处理—协变量交互作为平行斜率判断。[^psu-ancova-slopes]
...
单因素模型中，\(SS_A=\sum_i n_i(\bar Y_i-\bar Y)^2\) 已经正确反映不等样本量，均值差标准误也应使用实际的 \(n_i,n_j\)。用一个合成的 \(n_0\) 代替所有组样本量、再套用统一 LSD 或极差标准误会掩盖不同比较的精度。现代计算直接使用模型参数的协方差矩阵表达这种不平衡。

在总样本量和共同方差固定、所有组同等重要时，平衡配置通常使比较精度均匀并保持效应正交。不同组成本、方差、失访或科学优先级也可能支持有意的不等分配。分析需要说明不平衡如何产生、是否与结局或处理有关，以及它怎样改变待估总体和精度。
...
| Type III | 每个效应在模型中其余效应，包括包含它的交互之后，以可估函数检验 | 需要明确对比编码、边际权重和可估性；其含义由待检验函数决定 |

SAS 的一般线性模型（general linear model，GLM）文档明确指出，Type I 是顺序平方和且在不平衡数据中依赖项顺序和单元格数；Type II 不依赖书写顺序，但含有效应会进入其可估假设；Type III/IV 通过可估函数构造，在无空单元格时相同。[^sas-sums-of-squares] 因此应先定义科学问题和待检验函数，再选择相应平方和类型。
...
若某些因素组合没有任何观测，部分对比可能无法由数据唯一估计。软件给出的别名、奇异或不可估（non-estimable）提示表明相应设计证据缺失。此时需要缩小推断范围、合并有科学依据的水平、补充数据，或改用能明确表达结构的模型。
...
正态性针对误差或条件残差，各组混合后的边缘响应可以呈现其他形状。分位数—分位数图（quantile–quantile plot，Q–Q plot）检查尾部和离群，残差—拟合值图检查方差随均值变化与模型形状，按处理绘制原始点和残差可发现特定组问题。NIST 的单因素 ANOVA 指南至少建议检查按运行顺序的残差、正态概率图以及残差—拟合值图。[^nist-oneway-diagnostics]

平衡且各组样本量适中时，均值 F 检验对轻度非正态往往较稳健；方差不等与样本量不等同时出现时则可能严重扭曲错误率。离群值既可能是记录错误，也可能是真实生物异质性，删除前应追溯实验记录并报告规则。正态性检验和 Levene 检验的检出能力受样本量影响，需要与图形、设计和模型诊断结合解释。
...
强偏斜、重尾或少量离群下，可以报告稳健位置估计、自助法（bootstrap）区间或稳健 ANOVA；科学目标是秩位置或随机优势时，可使用 [Kruskal–Wallis](categorical_nonparametric.md#rank-methods)；随机分派实验还可在设计允许的单位内置换 F 或对比统计量。伪重复、错误误差层级、处理后选择或非随机缺失仍需在设计和模型中分别处理，各替代方法对应的效应参数也需明确。
...
ANOVA 分析从设计图开始：标出响应、因素与水平、处理组合、实验单位、随机化层级、区组、重复和协变量；再写出目标均值或对比及其可推广总体。随后查看各单元原始点、样本量和缺失，建立包含必要交互与误差结构的模型，检查可估性与残差，最后按事前比较家族给出估计和推断。模型选择围绕科学问题进行，平方和类型、变换和事后检验各自服务于预先定义的目标。

结果至少报告：每个单元的分析单位数、均值与标准差或其他适当描述；完整模型公式与固定／随机效应角色；平方和类型、误差项、自由度、F 与精确 *p* 值；主要对比或估计边际均值的权重、效应估计与置信区间；多重比较家族及调整方法；效应量公式；残差和敏感性分析；缺失、排除及不平衡原因。字母分组或“ANOVA 后做 LSD”只概括部分比较结果，以上信息才能重建完整分析。

ANOVA 的因果解释来自处理随机分派，向总体推广来自抽样与研究场景。模型把观察变异分解成若干数学来源，生物机制则需要结合实验设计和外部证据解释。可靠结论应把变异分解、效应大小、实验设计和机制证据重新合在一起。
