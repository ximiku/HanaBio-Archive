...
统计推断从观察总体的一部分开始，再用这部分资料认识总体。推断范围由样本量、总体界定、入样机制、单位间的独立性以及统计量的抽样波动共同决定。一次样本给出的均值或比例只是一个实现值；若按同一规则重新抽样，结果通常会改变。

抽样分布把这种“可能怎样改变”写成概率分布，标准误度量统计量的抽样波动，参数估计则把样本信息转化为未知总体量的点估计和区间估计。三者共同构成从[概率分布](probability.md)进入统计推断的桥梁。下面由样本均值、Student \(t\)、\(\chi^2\) 和 \(F\) 分布的经典推导进入抽样设计、估计量性质和现代重抽样方法。

## 样本与总体的抽样联系 { #sampling-design }

### 目标总体、抽样框与入样机制

抽样首先要说明结论指向的**目标总体**。研究某湿地繁殖鸟类的平均巢卵数时，目标总体需要同时限定物种、地点、繁殖季和可观察对象；研究培养条件下细胞的平均表达量时，总体可能是该实验流程能够产生的独立培养物，显微图像中的细胞则是培养物内的观察单位。目标总体决定参数的含义，也限定结论的推广范围。

**抽样框**是实际用于选择单位的名单、空间网格、时间段或其他操作性集合。抽样框遗漏偏远样地、只包含就诊个体或重复登记某些对象时，框总体与目标总体之间出现覆盖误差。随机选择在既有抽样框内控制选择机制，框外对象的代表性还需另作论证。

在概率抽样中，每个总体单位有已知且大于零的入样概率 \(\pi_i\)。这种可描述的随机机制使设计型的总体估计和抽样误差估计成为可能。便利样本、志愿者样本和配额样本也能提供有价值的数据，但入样机制通常与结局、可及性或参与意愿有关；把它们推广到更广总体时，需要额外的模型、辅助资料和敏感性分析，样本规模主要决定既定入样机制下的随机精度。

### 常见概率抽样设计

概率抽样的几种基本形式解决不同的抽样框与成本问题。加拿大统计局（Statistics Canada）对这些方法的正式说明强调：分层抽样在每个层内独立抽取单位，整群抽样则先抽取若干群，二者具有不同的选择结构。[^statcan-sampling]
...
分层的目标是让每个层都得到信息，整群的目标则常是访问部分群。生命科学调查经常把两者组合：先按生境分层，在每层抽取样地，再从样地内抽样方或个体。此时“样本量”至少要分别报告抽中的层、群和末级单位数，才能呈现真正独立的抽样层级。
...
霍维茨—汤普森（Horvitz–Thompson，HT）总量估计量为
...
在入样概率已知且设计条件满足时，它对有限总体总量无偏；总体大小 \(N\) 已知时，可用 \(\widehat Y_{\mathrm{HT}}/N\) 估计总体均值。实际调查还可能因无应答、覆盖校准和后分层调整权重。点估计和标准误都要按设计处理，加权均值的方差计算也需保留分层、整群与入样概率等结构。
...
根号内的 \(1-n/N\) 是有限总体修正。抽样比例 \(n/N\) 很小时它接近 1；抽样接近普查时，剩余抽样不确定性随之下降。这个修正针对有限总体不放回抽样，选择偏倚、无应答和测量误差需要分别处理。
...
均值标准误为 \(\sigma/\sqrt n\)：均值的方差先成为 \(\sigma^2/n\)，开方后得到这一结果。总体标准差未知时，常以 \(s/\sqrt n\) 估计标准误。这个代入又引入了估计 \(\sigma\) 的不确定性，正是 Student \(t\) 分布出现的原因。
...
单个数给出的估计称为**点估计**。置信区间的下限和上限是按覆盖要求构造的区间端点，单侧置信上限或下限也是相应方向的置信界。点估计描述估计目标的位置，区间端点表达其不确定性范围。
...
无偏只描述重复抽样的平均中心，某一次估计与真值的距离还取决于方差。一个方差很大的无偏估计量可能比带有少量偏差但稳定得多的估计量更不实用。均方误差把两方面合在一起：
...
估计量还常按一致性、效率、稳健性和对模型误设的敏感程度评价。**一致性**表示样本信息增加时估计量依概率趋近目标参数；它是渐近性质，小样本表现还需另行评估。估计量的优劣需结合数据生成机制和损失目标判断。

### 矩估计与最大似然估计

矩估计把样本矩与理论分布矩对应起来。例如泊松分布满足 \(E(X)=\lambda\)，令样本均值等于理论均值便得到 \(\hat\lambda=\bar x\)。这种方法直观，常能给出闭式解；其效率取决于所用矩包含了多少模型信息。
...
似然比较同一组观测对不同参数值的相对支持。要表述“给定数据后参数取某值的概率”，还需要另行规定参数的概率模型和先验，将在[贝叶斯推断与计算统计](bayesian_computational.md)中讨论。
...
收敛速度取决于偏斜、尾部、离散性和相关结构。极端重尾、强依赖或少数群贡献大部分信息时，普通近似可能很慢或不适用，因此样本量需要结合这些性质判断。若原始观测本来独立服从正态分布，则 \(\bar X\) 在任何 \(n\) 下都精确正态，可直接使用正态分布结论。
...
Student \(t\) 分布关于 0 对称；自由度较小时，估计尺度带来的不确定性使尾部比标准正态更厚，自由度增加时逐渐接近标准正态。这里的精确结论依赖独立正态样本。对非正态数据，\(t\) 方法的近似质量取决于样本量与分布形状，需要结合诊断判断。
...
\(F\) 分布也只取非负值，其形状同时由分子和分母自由度决定。它的均值在 \(\nu_2>2\) 时为 \(\nu_2/(\nu_2-2)\)；这一矩性质与分布在 1 附近的相对集中是不同概念。NIST 的 \(t\)、\(\chi^2\) 和 \(F\) 条目给出了这些分布的支持范围、自由度依赖和矩条件。[^nist-pivots]
...
或在近似意义下接近这一覆盖率。重复按同一设计抽样并构造区间时，长期约有 \(1-\alpha\) 的区间覆盖固定参数。对一次已经得到的区间，固定参数与区间的关系只有覆盖或未覆盖两种；“95%”描述的是构造方法的长期覆盖率。NIST 也以这一频率学派含义解释置信度。[^nist-confidence]

区间宽度同时受置信水平、标准误、样本量和模型影响。提高置信水平通常使区间变宽；增加独立信息常使标准误下降。更窄表示在给定方法和假设下抽样精度更高，抽样框覆盖、测量准确度和模型偏差则需分别评估。
...
这三个区间对应三种不同设计，选择时应依据两组独立、单组均值或配对差值的实际结构。偏斜很强的小样本、删失数据、明显层级相关或选择性排除会破坏简单 \(t\) 区间的条件，需要采用与设计和估计量相符的方法。
...
在样本不大或 \(p\) 接近 0、1 时覆盖率可能很差，甚至给出小于 0 或大于 1 的端点。\(n\hat p\) 与 \(n(1-\hat p)\) 可以提示近似条件，实际覆盖率还取决于具体的 \(n\) 与 \(p\)。

经典正态近似有时在比例尺度的误差界限上加入 \(1/(2n)\)，以连续性校正补偿离散二项计数与连续正态面积之间的差别。这一修正的效果随 \(n\) 和 \(p\) 改变，需要实际评估覆盖率；比例统计量的参照仍来自二项分布及其近似。现代计算条件下，更适合直接采用得分型或二项尾概率区间。
...
Clopper–Pearson 区间通过反演精确二项尾概率构造，以二项分布而非正态近似为参照；由于二项分布离散，覆盖率常高于名义水平而显得保守。复杂抽样中的加权比例、风险差或两个比例之差还需要设计型、得分型或模型型区间，以匹配相应估计目标。
...
这里 \(\chi^2_{q,\nu}\) 表示自由度 \(\nu\) 的 \(q\) 分位数。区间不以 \(s^2\) 对称，因为 \(\chi^2\) 分布右偏。它对非正态尤其是重尾数据较敏感，使用时需要检查正态样本条件。

区间形式总由参数、估计量和抽样分布共同决定。均值、比例、方差、分位数、发生率和模型系数各有相应构造；即使写成“估计值 ± 临界值 × 标准误”的外形，临界分布和标准误仍必须与设计及模型相配。

## Bootstrap 重抽样 { #bootstrap }

### 从经验分布近似抽样分布

某些统计量的抽样分布难以解析推导，例如中位数、偏斜分布的比值或复杂模型预测。非参数自助法（bootstrap）把观测样本的经验分布当作总体分布的近似，从原样本中有放回地抽取 \(n\) 个单位，得到自助样本；重复 \(B\) 次并计算
...
百分位区间直接取自助重复值的 \(\alpha/2\) 和 \(1-\alpha/2\) 分位数。基本区间把自助分布偏离原估计的方向反射回参数尺度；学生化区间（studentized interval）还对每次重复值按其标准误标准化。偏差校正加速区间（bias-corrected and accelerated interval，BCa）进一步校正自助分布相对原估计的偏移与变化率。DiCiccio 与 Efron 的综述比较了这些区间方法，各方法具有不同的校正目标。[^bootstrap-ci]

自助法重用原样本中的信息，也继承其覆盖范围。样本未覆盖的亚群不会由重抽样生成，极小样本提供的信息仍然有限，边界参数和不连续统计量可能产生退化的重抽样分布。\(B\) 有限还带来蒙特卡洛（Monte Carlo）误差，应检查区间端点是否随随机种子和重复次数稳定。

重抽样单位必须与独立信息单位一致。配对资料应重抽整对观测，整群设计应重抽群或按设计分阶段重抽，时间序列常需区块自助法；把同一动物的细胞、同一培养皿的孔或连续时点逐行独立重抽，会把原有层级相关误作独立信息。参数自助法则从拟合模型模拟数据，效率可能更高，其有效性也更依赖模型设定。

## 从估计结果回到研究问题 { #reporting-estimates }

一份可解释的估计结果应同时写明目标总体与待估量、抽样框与抽样设计、独立单位和层级、点估计、标准误或置信区间（confidence interval，CI）、区间方法及关键条件。复杂调查还要报告权重、分层、整群和有限总体处理；自助法结果要说明重抽样单位与区间类型。这样读者才能由“均值为 12.4，95% CI 见图”等结果判断区间覆盖的是哪个总体参数。

抽样不确定性是总不确定性的一部分。普通标准误主要量化既定模型下的抽样波动；覆盖不足、无应答、测量偏倚、批次效应、缺失数据和模型误设需要分别评估。增加同一入样机制下的样本量可以使区间更窄，估计所对应的目标仍由抽样设计决定。抽样设计建立从样本到总体的通道，估计方法再量化这条通道中的随机波动。

置信区间需要与测量单位、生物学阈值和研究设计一起解释。区间是否跨过零连接相应的双侧检验，效应大小、实际重要性和多重比较语境则提供科学解释。这些问题将在[假设检验、效应量与多重比较](hypothesis_testing.md)中继续展开。
