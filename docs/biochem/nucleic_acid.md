...
### 游离核苷酸的代谢、基团转移与信号功能 { #free-nucleotides }
...
### 双螺旋稳定性与沟槽识别 { #dna-duplex-stability-recognition }
...
碱基对相对螺旋轴的偏移使双链表面形成大沟和小沟。每一类碱基对在沟面暴露不同的供氢体、受氢体、甲基和氢原子组合，蛋白质因而能在保持整体双链的情况下读取序列；大沟通常提供更丰富的判别图样，小沟的宽度、静电势和水化模式也能被蛋白质和小分子识别。DNA 的局部几何随序列改变，包括扭转、滚转、弯曲和沟宽，蛋白结合还可诱导弯折、解旋或碱基翻出。PDB-101 汇集的结构展示了这些局部变化以及 A、B、Z 三类规则螺旋的差异。[^pdb-dna]
...
### 规则构象与非经典结构 { #dna-conformation-families }
...
### RNA 二级与三级结构 { #rna-secondary-tertiary-structure }
...
远距离相互作用把这些局部元件装配为三级结构。螺旋同轴堆积、发夹—受体结合、A-minor 接触、核糖拉链、碱基三联体和假结都可稳定特定折叠；Mg$^{2+}$ 等离子既屏蔽骨架负电，也可通过内层或外层配位组织局部结构。RNA 虽常以单链合成，却可通过 Watson–Crick、Hoogsteen 型接触和碱基三联体形成复杂配对。结构数据库中的 motif 分类帮助比较反复出现的几何模块，实际 RNA 往往把多个 motif 重叠组合。[^rna-tertiary-motifs]

### RNA 功能类别与催化 { #rna-functional-classes-catalysis }
...
### DNA 拓扑与超螺旋 { #dna-topology-supercoiling }
...
### 核酸—蛋白质复合体 { #nucleic-acid-protein-complexes }
...
### 核酸沉淀与黏度的影响因素 { #precipitation-viscosity }
...
### 熔解温度的条件依赖性 { #melting-temperature }
...
### 复性动力学、互补性与序列复杂度 { #renaturation-kinetics }
