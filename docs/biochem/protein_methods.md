...
蛋白质研究并没有一套可以从任何样品直接通向“真实结构与功能”的通用流程。细胞裂解液中的目标蛋白可能只占总蛋白的很小一部分；把它纯化出来，又可能丢失原有配体、脂质、修饰或亚基。电泳条带、色谱峰、质谱离子和三维密度图分别是蛋白质在不同实验条件下留下的投影。方法学的核心因而不是记住仪器清单，而是让样品状态、分离依据和最终要回答的问题彼此相容。

一条可靠的证据链通常从保持目标状态开始，经过逐步分级和定量，再用相互独立的方法确认身份、纯度、结构与活性。下面的流程不是固定配方，而是说明这些环节之间的依赖关系。[^protein-fractionation-textbook]
...
## 样品状态决定方法上限 { #sample-state }

### 提取不是单纯地把蛋白质放进水中 { #protein-extraction }

组织或细胞被破碎后，原本分隔的蛋白酶、氧化还原体系和底物进入同一溶液，稀释又会削弱亚基、配体和辅因子之间的可逆结合。提取缓冲液要围绕目标蛋白设计：pH 应落在蛋白稳定且满足后续分离的范围，离子强度既要抑制非特异聚集，也不能破坏必要的静电相互作用；温度、金属离子、甘油或特定配体则按已知稳定条件选择。全程低温可以减慢许多降解与变性过程，却不能代替快速操作和针对性抑制。
...
DTT、β-ME 和 TCEP 都可能改变蛋白质的天然共价拓扑，也可能干扰基于铜离子或游离巯基的测定。因此“还原得更完全”并不总等于“样品更接近研究目标”。[^reducing-agents]

### 膜蛋白需要替代原生脂双层 { #membrane-protein-preparation }
...
## 分级纯化是一条定量路径 { #purification-path }

### 粗分级先降低样品复杂度 { #coarse-fractionation }
...
### 纯化表把损失与富集同时写出来 { #purification-metrics }
...
## 色谱把不同分子性质转化为保留差异 { #chromatography }
...
### 体积排阻读出的是流体动力学尺寸 { #size-exclusion }
...
### 正交步骤优于重复使用同一性质 { #orthogonal-purification }
...
## 电泳分开的是实验条件下的迁移物种 { #electrophoresis }
...
SDS-PAGE 中的多肽不是统一的“近球形小球”，而更接近被 SDS 包覆的延展链。多数蛋白在适当凝胶浓度下，其相对迁移率与摩尔质量对数近似线性，因此可与标准品比较；糖基化、强疏水跨膜区、异常 SDS 结合或不完全还原都会造成表观质量偏移。Laemmli 在 1970 年发表的不连续缓冲 SDS-PAGE 体系把样品先集中成窄带再进入分离胶，成为常用格式，但“常用”不意味着这些近似对每种蛋白都严格成立。[^sds-page-laemmli]
...
## 抗体把分子识别转为可测信号 { #antibody-methods }

### 多克隆与单克隆描述的是抗体群体来源 { #polyclonal-monoclonal }
...
### Western blot 把大小分离与表位识别串联 { #western-blot }
...
## 总量、组成与序列是不同层次的测量 { #quantity-composition-sequence }

### 蛋白质定量必须匹配样品化学 { #protein-quantification }
...
### 氨基酸组成分析始于有偏的水解 { #amino-acid-analysis }
...
### 端基化学开启了逐步测序 { #terminal-sequencing }
...
Edman 降解在弱碱条件下让苯基异硫氰酸酯（PITC）与游离 N 端反应，随后在较温和酸性条件下选择性释放首个残基并转化为可鉴定的 PTH-氨基酸，剩余肽链可进入下一循环。Edman 于 1949 年发表初步方法，1950 年由他单独发表详细反应；1967 年 Edman 与 Begg 报道自动蛋白质顺序分析仪。每轮偶联、裂解、转移和鉴定都不是百分之百，副产物会随循环累积，N 端封闭、样品混合和蛋白过长都会使读序失败。[^edman-history]

### 特异裂解把长链转成可解析肽段 { #protein-cleavage }

胰蛋白酶通常在 Lys 或 Arg 的羧基侧裂解，但后接 Pro、邻近修饰和三级结构可改变效率；糜蛋白酶偏好芳香族及部分疏水残基附近，Glu-C、Lys-C、Asp-N 和胃蛋白酶则有各自条件依赖的选择性。不能把“所有蛋白酶都不裂解与 Pro 相邻的肽键”写成普遍定律。化学裂解中，甲硫氨酸羧基侧的经典试剂是溴化氰（CNBr），不是溴化氢；其副产物和甲硫氨酸氧化状态也会影响反应。
...
### 质谱把离子质量差转为序列证据 { #mass-spectrometry }

质谱仪直接测量的是离子的质荷比 $m/z$，不是一个不带电分子的“重量”。同一肽可形成多个电荷态，同一元素或肽也会产生同位素峰簇；旧笔记中“绝不会有两个核素质量呈整数倍关系”既不是质谱分离原理，也不是可靠判据。软电离产生肽离子后，一级质谱给出前体离子的 $m/z$；串联质谱再选择前体、诱导碎裂并测量产物离子，肽键断裂形成的系列碎片提供序列约束。
...
## 光谱与三维结构方法观察不同尺度 { #spectroscopy-structure }

### 溶液光谱追踪平均构象 { #solution-spectroscopy }
...
### 原子模型是数据约束下的解释 { #structural-methods }
...
## 固相肽合成把序列假说变成可测分子 { #solid-phase-peptide-synthesis }
