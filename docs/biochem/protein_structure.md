...
一条多肽链并不是把氨基酸串成的柔软细线。肽键使主链局部趋于平面，侧链又以不同的体积、电荷和溶剂相互作用限制相邻二面角；沿序列反复出现的这些局部条件，最终在水、膜、离子、配体和其他大分子共同构成的环境中形成三维构象。

传统的一级、二级、三级和四级结构为描述蛋白质提供了共同语言，却不意味着折叠必然依次完成四个台阶。真实蛋白质会呼吸、摆动、交换构象，有些区段只在结合伙伴出现时才有序，也有些蛋白质本来就以动态构象集合工作。因而“结构”既包括氨基酸序列和稳定折叠，也包括共价修饰、亚基装配、柔性与环境依赖的构象平衡。
...
这四层术语仍然有用，但它们是观察同一分子的不同尺度，而不是四种彼此独立的作用力。[^protein-structure-levels]
...
主链的大部分构象变化来自绕 $mathrm{N-C_{\alpha}}$ 与 $mathrm{C_{\alpha}-C(=O)}$ 键的旋转，分别用 $\phi$ 和 $\psi$ 表示。肽键平面并没有把多肽变成刚性杆；它只是把任意旋转压缩为一组受立体位阻、氢键、静电和溶剂条件共同筛选的构象自由度。
...
允许区表示几何上较可行，并不保证某个残基在给定蛋白质中一定稳定；落在少见区域也不自动证明模型错误。残基类型、相邻原子、修饰状态和结构测定不确定性都会改变判断。高质量结构模型通常只有少数真正的离群点，每个离群点都应结合电子密度、局部化学或其他实验数据检查，而不能仅凭一张图机械删除。[^pdb-validation]
...
二级结构不是由某一种残基标签直接指定，而是许多相邻残基同时采用相容的二面角并形成协同相互作用的结果。α-螺旋在 Ramachandran 图上形成一簇，β-折叠股形成另一簇；转角和环区则可跨越多个区域。序列中的局部倾向有预测价值，却不能把“螺旋形成残基”或“β-折叠残基”当作孤立残基的固定属性。
...
Pro 会中断常规的主链氢键并限制二面角，Gly 则增加未折叠态自由度，因此两者在长 α-螺旋内部相对少见；它们在螺旋起始、终止、弯折或功能性铰链处却可能十分重要。残基倾向描述统计趋势，不是“能／不能形成螺旋”的二元名单。

### β-折叠片、转角与环区 { #beta-sheet-turn-loop }

β-折叠股是伸展的主链区段，多条折叠股通过主链羰基与酰胺之间的氢键并排组成 β-折叠片。相邻股的 N→C 方向相同称平行，方向相反称反平行，也可在同一折叠片中混合；两类氢键几何不同，但都广泛存在，不能概括为平行片必定埋藏、反平行片必定暴露。侧链沿折叠股交替伸向折叠片两侧，交替出现疏水和亲水残基时可以形成两亲性表面。[^secondary-structure-textbook]
...
不能归入 α-螺旋或 β-折叠片的片段通常称环区或 coil。一个解析清楚的环可以具有稳定、专一的构象，参与配体入口、催化位点和结构域运动；“无规卷曲”只说明缺少规则重复的二级结构，不等于原子毫无组织。相反，内在无序区在游离状态下可能真正以快速交换的构象集合存在，并在结合或修饰后改变其分布。DisProt 只收录有实验依据的内在无序信息，提示低复杂度序列、未解析坐标或低预测置信度本身都不能单独证明生理性无序。[^disprot-idr]
...
### 稳定结构的是相互作用网络 { #structure-stabilizing-interactions }

蛋白质折叠要比较整个体系的自由能，而不是把若干“强键”简单相加。疏水基团从水中聚集可释放部分有序水分子，是许多可溶性球状蛋白形成核心的重要驱动力；主链和侧链之间的氢键、离子相互作用与紧密堆积则提供几何选择性。每一种贡献都受溶剂暴露、介电环境、温度、盐浓度和构象熵影响。[^protein-shape-textbook]

| 相互作用 | 在结构中的典型作用 | 不能脱离的条件 |
| --- | --- | --- |
...
一条长多肽通常不是均匀折成一个团块。局部二级结构先组合成反复出现的空间模式，较大的区段又形成相对独立、可协同运动的结构域；结构域之间的连接使结合、催化和别构调节可以在同一蛋白质中模块化组合。
...
球状、纤维状和膜蛋白描述总体形态及所处物理环境，而不是互斥到没有例外的化学类别。可溶性球状蛋白常把疏水表面埋入内部；跨膜蛋白则把疏水侧链朝向脂双层，并以孔道或内部极性网络容纳水、离子和底物。膜蛋白需要去污剂、纳米盘或脂质等模拟其环境，并不是简单“溶于脂质”。肌动蛋白和微管蛋白的单体是球状蛋白，却分别能聚合成长丝和微管，因此宏观纤维形态不等于单体属于经典纤维状蛋白。
...
胶原三股螺旋不是 α-螺旋，也不能说“胶原既不是左手又不是右手螺旋”：单链与三链超螺旋分别具有明确且相反的手性。原素材对胶原结构的组织经 osm.bio 细胞外基质条目启发后重新核对，并以 NCBI Bookshelf 的 Gly–X–Y、羟化、分泌加工和纤维装配说明交叉校正。[^collagen-structure]
...
寡聚化能够把活性位点分布在亚基界面、增加结合位点之间的协同性，或允许亚基之间传递构象变化；这类性质将在[蛋白质功能](protein_function.md)中结合血红蛋白和分子识别展开。它们是特定序列和装配产生的后果，并不表示多亚基结构天然“优于”单链结构。许多蛋白质以单体工作，另一些则在单体、二聚体和更高聚体之间随浓度、配体、膜环境或修饰状态动态交换。
...
新生肽可在仍连接核糖体时开始折叠。分子伴侣通过遮蔽暴露的疏水表面、隔离折叠空间或利用 ATP 驱动结合循环来降低聚集风险，并不通常把一套外源结构信息写入序列。蛋白质二硫键异构酶催化氧化、还原与二硫键重排，肽基脯氨酰顺反异构酶加快可能较慢的 X–Pro 肽键异构化；两类酶改变动力学路径，却不等于天然构象完全由它们指定。

### 变性与复性 { #denaturation-refolding }

升温、极端 pH、尿素、盐酸胍、有机溶剂或去污剂可改变蛋白质—水和蛋白质内部相互作用，使天然构象群转向展开态或其他非天然态。通常的变性不切断主链肽键，因此一级序列保留；若同时加入还原剂、强氧化剂或发生水解，共价结构也可能改变。黏度、溶解度、沉降、圆二色性、荧光和活性的变化都依赖具体蛋白质及条件，不存在对所有蛋白都适用的“三增三减”。
...
### 实验结构不是分子的直接照片 { #experimental-structure-methods }

X 射线晶体学、溶液 NMR 和单颗粒冷冻电镜从不同观测量约束结构模型。它们提供的是“数据支持下的坐标解释”，而不是无条件精确的原子照片；样品状态、空间分辨率、构象异质性和建模选择都会进入最终模型。详细样品制备、分离、光谱和测序流程见[蛋白质研究方法](protein_methods.md)。
...
### 读取坐标文件的缺口与不确定性 { #coordinate-model-uncertainty }
...
## 从结构描述走向机制 { #structure-to-mechanism }
