...
酶的活性位点把底物、催化残基、结构水、金属离子和辅因子安排在同一微观环境中。这里发生的并不是一种名为“酶催化”的单独作用力，而是若干化学贡献在时间和空间上的配合：底物进入可反应的取向，质子和电子沿特定路径转移，短暂中间体被生成又被及时拆除，最终产物离开而酶回到可继续周转的状态。
...
## 活性位点是选择反应路径的微环境 { #active-site-microenvironment }

活性位点通常只占酶表面或内部的一小片三维区域，却可汇集一级序列上相距很远的残基。结合亚位点负责识别底物的形状、电荷和立体方向，催化基团直接参与成键、断键或质子转移；两者可以部分重叠，但“结合得紧”并不等于“催化得快”。如果酶同样强地稳定反应物基态和过渡态，结合能甚至可能把底物困在不能高效周转的状态。催化的关键是相对于反应物态，选择性降低通往过渡态的自由能代价。[^active-site-catalysis]

底物结合可减少两个反应物彼此寻找以及调整取向所需的熵代价，并把亲核体、亲电中心和质子供受体放到可反应的距离。疏水口袋、局部电场、氢键网络和排布有序的水又会改变基团的溶剂化、极化与表观 $pK_a$。这些贡献不能简单相加为一份适用于所有酶的“效率清单”：同一项相互作用若更偏爱基态而非过渡态，未必产生净催化优势。
...
多数酶并非只采用一种策略。下表的分类描述催化循环中的化学角色，而不是互斥的酶家族。
...
共价中间体不是过渡态。它占据自由能曲线上的局部低点，可以在快速淬灭、质谱或晶体结构中被捕获；形成和拆除中间体时则各要越过新的过渡态。好的共价催化剂并非把中间体稳定到极限，而是让整个替代路径的最高能垒低于未催化路径。

### 金属离子催化 { #metal-ion-catalysis }

金属离子可作为 Lewis 酸吸引电子密度、稳定负电荷、给底物定向，并降低配位水失去质子的代价；Fe、Cu、Mn 等还能改变氧化态，参与电子转移。人碳酸酐酶 II 的 Zn$^{2+}$ 由三个 His 和水或氢氧根近四面体配位。锌结合的氢氧根进攻 CO$_2$ 生成碳酸氢根，随后 His64 与有序水网络把质子送往体相并再生亲核体。这一例子同时包含金属、酸碱、静电和水网络贡献，不能缩写成“锌离子本身催化”。[^carbonic-anhydrase]
...
水进入活性位点后，His57 再次作一般碱活化水。水的氧进攻酰基—酶，经过第二个四面体中间体后断开 Ser195 与底物的共价键，释放另一产物。这个循环中，Ser195 承担共价催化，His57 往返承担酸碱催化，识别口袋和氧阴离子穴分别选择底物基态的取向和高能态的电荷。所谓“催化三联体”因此不是三个残基各自贡献一个可独立相加的倍率，而是一条耦合的质子与电子重排路径。
...
## 从活性残基候选到机制证据链 { #mechanistic-evidence }
...
## 别构作用与协同性并非同义 { #allostery-cooperativity }
...
协同性则是一个配体结合事件改变后续同类配体结合或催化响应的现象，常见于多位点蛋白。它可与别构相伴，却不存在必然联系：异促效应物可以调节一个单体酶而没有同类位点协同，多位点底物响应也不必来自一个传统“调节中心”。动力学上，K 型效应主要移动半最大响应浓度，V 型效应主要改变限制速率，真实体系常兼有两者；这些是数据表现，不等同于唯一结构机制。[^allostery-terms]
...
## MWC 与 KNF 是两种限制模型 { #allosteric-models }
...
大肠杆菌天冬氨酸氨甲酰转移酶（ATCase）展示了催化与异促调控的结构连接。完整酶含六条催化链和六条调节链；CTP 结合调节链抑制嘧啶合成通路，而底物及双底物类似物 PALA 偏向稳定活性较高的 R 状态。PALA 复合物结构显示，活性位点还接收相邻催化链环区的相互作用，因此调节链与催化链的相对移动可以同时改变多个活性位点。它是 MWC 思想的经典例子，却不代表所有别构酶都必须是十二聚体或都执行终产物反馈。[^atcase-structure]

## 可逆修饰与蛋白质相互作用 { #reversible-regulation }

蛋白质磷酸化由蛋白激酶把 ATP 的磷酰基转移给受体残基，去磷酸化则由蛋白磷酸酶水解磷酸酯。加入磷酸基会改变局部电荷、氢键和蛋白质相互作用，可能稳定活性态，也可能稳定低活性态；同一蛋白的不同位点还可产生相反或条件依赖的结果。磷酸酶不是简单“取消信号”的被动清除器，它与激酶共同决定修饰占有率、持续时间和空间分布。[^protein-phosphorylation]

糖原磷酸化酶是经典实例：磷酸化促进其活性态，同时 AMP、ATP、葡萄糖等代谢物还可按组织和同工形式施加别构影响。这个体系说明共价修饰与别构并非互斥层次。乙酰化、腺苷酰化、ADP-核糖基化和氧化还原修饰也能改变酶活性，但每种修饰都有自己的供体、写入与去除反应，不能把“共价修饰”一概写成同一种可逆开关。
...
酶原是需要特定加工才形成充分活性构象的前体，常见于消化蛋白酶、凝血和补体级联。有限蛋白水解切开一条或少数几条特定肽键，新的末端、盐桥和构象重排可形成或暴露活性位点。前体不必在所有测定中绝对零活性，激活也不只是“一级结构决定三级结构”的静态演绎，而是一次改变共价主链并重新分配构象的加工事件。[^zymogen-regulation]
...
改变酶量提供另一时间尺度的调控。转录和翻译决定合成，蛋白降解决定寿命，区室转运和复合体装配决定有多少分子实际接触底物。与已有蛋白上的别构结合相比，重新合成常较慢，却没有“量变必然慢、质变必然快”的普适边界；快速降解、预存 mRNA 翻译和长寿命修饰都能改变这一排序。反馈抑制通常让通路产物作用于上游控制点，前馈调节则由较早的代谢物影响下游步骤，但控制点不必永远是第一步或所谓“限速酶”。净通量由整条网络、热力学和底物供给共同决定。
