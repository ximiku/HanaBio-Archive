...
肝糖原主要在餐后储存来自门静脉和肝内碳流的葡萄糖，在吸收后期与短期禁食中参与缓冲血糖。肝细胞把糖原产生的葡萄糖-6-磷酸（G6P）送入内质网的 G6Pase 系统，生成游离葡萄糖并输出；这一终末步骤与[糖异生](gng.md#g6pase-bypass)共用。肝糖原分解和糖异生的相对贡献随禁食时间连续变化，并存在较长的重叠阶段。
...
在经典模型中，糖原素（glycogenin）先把 UDP-葡萄糖的葡糖基共价转移到自身 Tyr 残基，再自糖基化形成一段短 $\alpha(1\rightarrow4)$ 葡聚糖；糖原合酶随后接手延长。糖原素由此为许多颗粒提供经典共价引发位点，并与糖原合酶相互作用；颗粒数量和糖原合酶活性还受其他装配因素影响。[^glycogenin-classic]
...
GBE1 进行链内转糖基作用，无须为每个分支另加 ATP。分支酶转移较长链段以建立结构；去分支酶则在分解受阻时先移动三个残基，再水解暴露的单个分支葡萄糖。
...
一个 AGL 多肽具有两种催化活性。4-$\alpha$-葡聚糖转移酶先把分支末端三个 $\alpha(1\rightarrow4)$ 连接的残基移到邻近非还原端，只在分支点留下一个葡萄糖；随后淀粉-$\alpha$-1,6-葡糖苷酶以水解切开该 $\alpha(1\rightarrow6)$ 键，释放游离葡萄糖。前一步属于转糖基，只有后一步属于水解。[^debranching]
...
收缩引起的 Ca$^{2+}$信号与 AMP升高能在激素变化之前迅速动员肌糖原。肝细胞则把葡萄糖本身、胰岛激素和神经内分泌信号汇合起来。“胰岛素促进合成、胰高血糖素促进分解”概括全身调节方向；组织内响应还由颗粒定位、PP1靶向亚基、酶同工型、训练与营养状态共同决定。运动中不同供能系统的相对贡献将在[肌细胞生理](../physiology/muscle.md)中展开，胰岛激素的分泌控制见[胰岛内分泌](../physiology/endo/endo_islet.md)。
...
“糖原贮积病”（GSD）包含合成、胞质分解、溶酶体降解、糖酵解和葡萄糖输出等不同缺陷，可表现为糖原减少、积累或结构异常。现行定位应同时注明基因、区室、组织和受阻反应；罗马数字编号仍保留历史沿革中的差异。[^gsd-overview]
...
肝糖原合酶缺陷通常归为 0 型，多种磷酸化酶激酶缺陷归入 IX 型谱系。VII 型 Tarui病由肌磷酸果糖激酶 *PFKM*缺陷导致，历史上虽纳入 GSD，却直接阻断糖酵解而非糖原末端切除，机制见[糖酵解](emp.md#nonequilibrium-control)。疾病表现、诊断和治疗须在[糖与能量代谢紊乱](../physiology/pathophysiology/metabolic_disorders.md)中结合临床证据讨论；这里的表格只用于定位反应层次。[^gsd-classification]
