...
蛋白质的功能并不是三维结构旁边附加的一句名称。一个分子能否在细胞中工作，取决于它识别什么配体、各构象之间怎样交换、是否与其他亚基装配、能量输入如何改变这些状态，以及它在何种区室和浓度范围内出现。同一条多肽因配体、翻译后修饰或结合伙伴不同，可以进入功能不同的分子状态；外形相近的蛋白质也可能因少数关键残基改变而选择完全不同的底物。

许多功能都从分子识别开始。结合能够把底物置于催化位点、使受体感知信号、让抗体包围抗原表位，也能把 ATP 水解与机械运动耦联。因而需要同时讨论平衡、速率、构象和细胞语境，而不能从一幅静态结构图直接推出完整机制。[蛋白质结构](protein_structure.md)提供这些讨论所需的构象语言；具体测量方法将在[蛋白质研究方法](protein_methods.md)中展开。
...
当 $[L]_{\mathrm{free}}=K_d$ 时，位点平均占据一半。这个结果不是所有结合曲线的通式：多位点协同、竞争配体、受体耗竭、偶联反应和多种构象并存都会使测得的常数成为依赖模型的表观量。尤其当配体并不过量时，总配体浓度不能直接代替游离配体浓度。
...
### 结合伴随构象重新分布 { #binding-conformational-redistribution }
...
结合释放的自由能还可传递到远离配体口袋的位置。若一个位点的占据改变另一个位点的亲和力或反应性，就出现别构耦联；效应既可为正也可为负，也不要求蛋白质一定具有多个亚基。血红蛋白展示了亚基之间的正协同性，而胰岛素则说明寡聚化还可服务于储存：胰岛 β 细胞分泌颗粒中高局部浓度与 $\mathrm{Zn^{2+}}$ 稳定六聚体，结合受体的主要分子种则是单体。装配状态因此是功能语境的一部分，不宜把二聚体或六聚体简单标为“无活性废态”。[^insulin-assembly]

## 血红素口袋与肌红蛋白的单点结合 { #myoglobin-heme-binding }

### 珠蛋白为血红素建立微环境 { #globin-heme-pocket }
...
游离血红素容易发生氧化、非特异聚集和一氧化碳强结合；珠蛋白口袋固定卟啉、限制配体进入角度，并以远端残基和局部极性环境降低 CO 相对于 $\mathrm{O_2}$ 的优势。蛋白质在这里并非被动包装辅基，而是在不改变铁—卟啉核心化学的前提下重新塑造选择性、反应速率和氧化还原稳定性。

### 双曲线来自无协同的单个位点 { #myoglobin-oxygen-curve }

以氧分压 $p\mathrm{O_2}$ 代替溶解氧浓度时，肌红蛋白的分数饱和度近似服从单个位点双曲线；其 Hill 系数接近 1，没有经典的亚基间协同性。氧结合仍会引起血红素铁和口袋的局部位移，但“发生构象变化”并不自动等于别构调节。

肌红蛋白的氧亲和力高于血红蛋白，在通常供氧充分的肌细胞中保持较高饱和度，并在局部氧分压降低时释放氧、缓冲氧供应和促进氧向线粒体方向扩散。它不是只有氧耗尽后才突然放氧，饱和度会随氧分压连续改变；具体贡献还取决于组织类型、肌红蛋白浓度和扩散条件。[^oxygen-binding-curves]

## 血红蛋白的协同与别构 { #hemoglobin-cooperativity-allostery }

### 四个位点形成相互通信的体系 { #hemoglobin-four-sites }
...
$P_{50}$ 是达到半饱和所需的氧分压，数值越低表示在该组条件下表观氧亲和力越高；$n_H=1$ 表示没有可检测的协同性，$n_H>1$ 表示正协同性。人血红蛋白在规定的标准条件下 $n_H$ 约为 2.7，而不是 4；Hill 系数衡量协同强度，并不等于亚基数，也不能把经验方程当作完整的逐步结合机制。[^oxygen-transport-quantitative]
...
### 异促效应物调节氧的装载与释放 { #hemoglobin-heterotropic-effectors }
...
一氧化碳直接与亚铁血红素竞争氧，并使尚未被 CO 占据的亚基偏向高亲和力状态；结果既减少可用位点，又使其余位点较难在组织处卸氧。高铁血红蛋白中的铁为 $\mathrm{Fe^{3+}}$，该位点不能结合氧，红细胞的细胞色素 b$_5$ 还原酶系统通常持续把少量高铁血红蛋白还原回亚铁态。HbA$_{1c}$ 则主要来自葡萄糖对 HbA β 链 N 端 Val 的非酶促糖化；它可反映一段时间的血糖暴露，却不是解释正常氧协同性所必需的别构态。详细气体运输、氧解离曲线和异常血红蛋白生理见[呼吸生理](../physiology/respiratory.md)与[血细胞生理](../physiology/blood/blood_cell.md)。[^hemoglobin-derivatives]

!!! warning "曲线左移并不等于供氧改善"

    氧亲和力升高可促进低氧分压下的结合，却也可能妨碍组织卸氧。评价一个变构效应时必须同时考虑肺部装载与外周释放，不能只比较“结合更紧”。

## 分子变异揭示结构—功能因果 { #hemoglobin-molecular-variants }

### HbS 暴露状态依赖的聚合界面 { #hbs-polymerization }

镰状细胞病中的 HbS 由 β 珠蛋白第 6 位 Glu 变为 Val（HBB p.Glu6Val）产生。突变没有直接替换血红素口袋残基，却在分子表面增加一处疏水“突起”；去氧 HbS 构象暴露相配的疏水接触区，使分子能够成核并组装成长纤维。纤维增长降低细胞内血红蛋白的溶解性、增加红细胞刚度并促成镰变，而不是四聚体从内部“压迫细胞膜”。氧合状态下关键界面不利于同样的聚合，所以一次表面替换通过构象状态和分子间装配放大为细胞表型。[^hbs-polymer]
...
### 地中海贫血改变亚基供给平衡 { #thalassemia-chain-imbalance }
...
HbS 说明一个界面突变可以创造异常聚合，地中海贫血则说明正确亚基即使各自能够折叠，化学计量失衡仍会破坏整体功能。更完整的遗传类型、基因剂量和人群层面解释留给[人类遗传与发育遗传](../genetics/developmental_human.md)，红细胞病理留给[血细胞生理](../physiology/blood/blood_cell.md)。

## 能量耦联与分子机器 { #energy-coupling-molecular-machines }

选择性结合不只用来“黏住”配体，还能安排反应的先后次序。马达蛋白把核苷酸状态、轨道亲和力和结构域运动耦联，使 ATP 水解的自由能偏置原本可逆的热运动。关键不是一次水解像爆炸一样直接推动分子，而是不同核苷酸状态改变能量景观，使结合、解离和构象转换按特定循环发生。

### 肌球蛋白的力化学循环 { #myosin-mechanochemical-cycle }

肌球蛋白家族共有的马达结构域同时包含肌动蛋白结合面和核苷酸口袋，轻链结合的颈部可充当杠杆臂，尾部则决定二聚、丝状装配或货物结合等不同功能。在一个简化的肌球蛋白循环中，ATP 与强结合的肌动—肌球蛋白复合物结合会降低肌动蛋白亲和力并促使分离；ATP 水解使马达进入预备构象；随后与肌动蛋白结合、无机磷酸释放和杠杆臂摆动相互耦联，ADP 释放后重新进入强结合状态。各步速率和力学响应随肌球蛋白类型、负载和调节蛋白而变，不能把这套顺序当作所有马达完全相同的动画。[^myosin-cycle]
...
### 可变区识别，恒定区连接效应 { #antibody-variable-constant-regions }

一分子典型免疫球蛋白由两条相同重链和两条相同轻链通过非共价作用与二硫键组成。每个臂端由一对重链、轻链可变结构域共同形成抗原结合位点，互补决定区的序列和环构象提供识别表面；较保守的恒定区维持整体架构，重链 Fc 区还可结合 Fc 受体或补体相关分子，把“识别了什么”连接到“随后发生什么”。抗体并不是整条分子每处都直接识别抗原。[^antibody-architecture]
...
## 膜环境与无序区扩展功能空间 { #membranes-disordered-regions }

### 膜蛋白把两侧环境耦联起来 { #membrane-protein-functions }

脂双层不是膜蛋白周围的惰性溶剂。跨膜区把疏水侧链朝向脂质酰基链，同时以内部极性口袋、连续水通路或离子配位位点处理亲水底物；膜电位、张力、厚度和特定脂质又可改变构象平衡。离子通道形成选择性孔道，载体在交替开放的构象间转运溶质，受体把一侧的配体结合传递到另一侧，泵和呼吸链复合物则把化学或电化学能与跨膜转运耦联。[^membrane-protein-function]

膜蛋白并非普遍低丰度，也不因位于疏水环境就天然难以形成结构。膜内主链的极性肽键常由 α-螺旋或 β-桶内部氢键满足，但侧链、脂质和水仍参与稳定。真正困难的是把蛋白移出原生膜后，原有的疏水表面容易在水中聚集，且原生脂质、亚基和构象状态可能丢失。去污剂、纳米盘、脂质体和结构测定的选择属于方法学问题，留待下一页集中比较。

### 内在无序是一组动态状态 { #intrinsic-disorder-function }
...
## 从分子活性到生物学功能 { #molecular-activity-biological-function }
