...
酶不只是一类“高效而专一的蛋白质”。多数酶确为蛋白质，部分 RNA 也具有催化能力；许多反应还依赖金属离子、有机辅因子或多亚基装配。酶的专一性不是绝对排他的标签，活力也不是脱离条件的常数。理解这些边界，才能把一个反应名称、一个 EC 编号或一次显色读数还原为可检验的催化过程。
...
## 催化改变反应路径而非平衡终点 { #catalytic-pathway }
...
酶本身不能把热力学上不利的反应变成自发反应。细胞常把它与 ATP 水解、离子梯度或氧化还原反应耦联，使多个反应的总自由能变化有利；又通过底物供给、区室化和酶量改变净通量。因而“酶不改变平衡”与“细胞能调节代谢方向”并不矛盾：前者描述给定反应的热力学，后者描述开放系统中的反应网络。

## 催化分子的物质组成 { #molecular-composition }

### 蛋白质与 RNA 都能形成催化中心 { #protein-rna-catalysts }
...
### 装配方式参与界定催化实体 { #enzyme-assemblies }
...
空间邻近可以缩短中间物扩散距离，保护不稳定中间体，或使底物在相邻活性位点间直接传递；但观察到共定位或蛋白质相互作用还不能单独证明底物通道化。通道化需要动力学、同位素、结构或重构证据显示中间物没有先与体相充分混合。细胞器、膜表面和可逆支架也能提高局部有效浓度，不要求所有相关酶永久结合成一个复合体。

## 反应命名与 EC 分类 { #enzyme-nomenclature }

酶的通用名称往往由底物或反应类型加“酶”构成，便于日常交流；系统名称则尽量把供体、受体和反应写清。EC 编号不是蛋白质家族编号，而是对**所催化总反应**的分类。一个 EC 条目可以对应来自不同物种、序列并不相同的蛋白质；同一种多功能蛋白也可能因具有多个独立催化活动而关联多个 EC 编号。[^ec-classification]
...
“合酶”“合成酶”“激酶”或“ATP 酶”是有用的名称线索，却不能代替反应方程和 EC 条目。例如旧分类曾把多种离子泵放在 ATP 水解酶下，现行 EC 7 则以转位这一主要结果分类；同名酶若底物、电子受体或整体化学计量不同，也可能分属不同条目。阅读数据库注释时应同时核对反应式、物种来源、亚细胞位置和证据类型。

## 底物识别与反应专一性 { #substrate-specificity }

### 活性位点是三维反应环境 { #active-site-recognition }

蛋白质酶的活性位点通常是表面沟槽、裂隙或内部口袋，只占分子的一部分，却可汇集来自不同序列区段的残基、结构水、金属和辅因子。底物先以氢键、离子作用、疏水作用和范德华接触等非共价力进入可反应排列；真正决定产物的，不只是“能否结合”，还包括哪条键被定向、哪些质子和电子能转移，以及哪一种过渡态受到稳定。结合位点与催化位点可以重叠，却不是概念上的同义词。
...
### 识别伴随构象重新分布 { #induced-fit-conformational-selection }

“锁和钥匙”模型保留了形状与化学互补这一重要直觉，但真实酶不是刚性模具。Koshland 提出的诱导契合强调，底物接触可驱动酶和底物进入更适合催化的构象；现代构象集合观点还允许底物先选择未结合酶已短暂采样的构象，随后再发生调整。两条路径可以连续出现，不能只凭结合前后两幅静态结构判定先后。[^induced-fit]
...
底物与酶形成许多弱相互作用时释放的结合自由能，可以用来抵偿去溶剂化、限制平移与转动、改变构象以及组织催化基团所需的代价。催化效率并不来自把基态底物“黏得越紧越好”：如果酶只对基态复合物异常互补，反而可能加深 $ES$ 的自由能井，使它更难到达过渡态。高效酶通常让相互作用在接近过渡态的几何与电荷分布中得到更充分的满足。
...
细胞并非把所有酶均匀分布在同一溶液相中。区室化使酶、底物和辅因子在局部相遇，膜又可把化学反应与跨膜电化学梯度耦联。多酶复合体、瞬时酶簇和支架可能协调连续步骤，但只有在中间物传递获得直接证据时，才应进一步称为底物通道化。[^cellular-enzyme-organization]

反应通量可在多个时间尺度上调节：改变底物和产物浓度、可逆结合别构效应物、共价修饰已有酶、有限蛋白水解激活酶原、改变亚基装配，或通过转录、翻译和降解改变酶量。同工酶催化相同总反应，却可具有不同动力学、调控、组织分布或细胞定位。后续页面将分别讨论这些机制；这里需要保留的原则是，酶活性受调节并不等于“酶自行调节生命活动”，而是酶分子处于细胞调控网络之中。

## 从天然酶到工程化催化剂 { #enzyme-engineering }

酶工程的起点可以是天然样品筛选、宏基因组挖掘或重组表达，随后再按用途改变催化剂本身或其工作环境。固定化把酶连接或包埋在载体上，便于回收、连续反应和重复使用，也可能提高操作稳定性；与此同时，传质阻力、取向限制和局部微环境会降低表观速率。化学修饰、介质工程和辅因子再生解决的是不同层次的问题，不能都归结为“让酶更稳定”。
...
把酶工程简单划成“第一至第四代基因工程”会混淆工具、尺度和目标。更清楚的描述应说明改造对象是序列、辅因子、装配、固定化载体还是整个代谢网络，随后用相同的反应式、测定条件和误差控制比较工程前后的催化表现。
