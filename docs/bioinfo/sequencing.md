...
测序仪并不直接交付“基因组”或“转录组”。它先让一批核酸分子经过文库构建和平台检测，把荧光、pH 或离子电流等物理信号转换为碱基序列，再为每个碱基或整条读段估计可信度。分析者得到的 read（读段）只是被仪器实际观察到的一段分子记录；参考基因组上的比对、转录本丰度和变异位点，都是在这些记录之上继续推断的结果。
...
一次测序首先定义“哪些分子有机会被看到”。基因组 DNA 可以随机片段化，外显子组需要先富集目标区，常规 RNA-seq 往往先完成 RNA 选择和逆转录，扩增子、RAD-seq、CLIP-seq 与亚硫酸氢盐测序又各自选择特定分子或改变其化学状态。提取、片段化、末端修复、连接和扩增属于实验流程，具体操作见[核酸提取、扩增与分子操作](../exptech/biochem_molecular/nucleic_acid.md)；生物信息学在接收数据时仍须知道这些步骤，因为文库选择已经决定了可观察范围和偏倚来源。
...
插入片段（insert）是接头之间来自样品的部分，read length 是从一端实际判读的碱基数，fragment length 则包括两端读段之间可能未被读到的区域。paired-end 测序从同一插入片段两端分别产生 R1 和 R2；两条 read 的方向、间距和共同分子身份能帮助定位重复区、识别异常连接并估计片段长度。paired-end 不是两个独立生物重复，R1 与 R2 也不能按行号以外的任意顺序重新配对。
...
短 read 易于高深度覆盖并适合多数计数与小变异任务，遇到长重复、高度相似的旁系区域、复杂结构变异或全长转录本时却可能缺少跨越信息。paired-end 和较长 insert 能增加片段约束，但两端仍无法直接给出中间未测序部分的碱基。后续比对或组装会把这种信息不足表达为多重比对、断点不确定或图结构分支；具体处理将在[序列比对与数据库搜索](sequence_analysis.md)与[基因组组装、变异与比较分析](genome_analysis.md)展开。
...
Oxford Nanopore 的 flow cell 在电阻膜中排列蛋白纳米孔，孔两侧电位差驱动离子电流。带接头的 DNA 或 RNA 由 motor protein 控制通过孔道，孔内同时占据的一组相邻核苷酸改变电流；仪器记录连续的 raw signal（常形容为 squiggle），basecaller 再用统计或神经网络模型把信号转换成序列。这里不存在“外切酶逐个切下碱基再逐个识别”的常规测序步骤，一次电流状态也不是只由一个孤立碱基决定。[^nanopore-principle]

纳米孔读长主要受进入孔道的分子长度和样品完整性限制，因而可以跨越长重复、结构变异、单倍型和全长异构体。逐分子误差具有碱基判读模型和序列语境依赖性；孔蛋白、试剂、运行软件与 basecaller 更新都会改变结果。数据交付时应记录这些版本，并区分 raw signal、basecalled read、单链读段以及由互补链或多分子信息形成的共识。
...
短读长与长读长并非固定的质量等级。短 read 的高分子计数和成熟误差模型适合深度、低频事件及成本受限的大队列；长 read 的跨越信息适合从头组装、结构变异、相位和全长转录本；HiFi、纳米孔和短 read 之间又有不同的长度、信号与系统误差。混合设计可以让长 read 建立结构骨架、短 read 提供额外深度，是否真正增益仍应由研究问题、参考质量、样本量和下游算法验证。

## 碱基判读、FASTQ 与 Phred 质量值 { #fastq-phred }

basecalling 是从仪器信号估计碱基序列的模型步骤。Sanger 的输入是毛细管峰图，Illumina 使用循环荧光图像，PacBio 解释 ZMW 光脉冲，纳米孔则解释连续电流。相同的原始信号在新的校准或模型下可能得到不同序列和质量值；保存许可与存储条件允许时，关键项目应保留足以重新判读的原始信号，并记录 basecaller 名称、版本、模型和参数。

FASTQ 把一条 read 的序列和逐碱基质量值放在同一条记录中。现代常见写法以四行组成一条记录：
...
第一行以 `@` 开始并保存 read 标识，第二行是序列，第三行以 `+` 开始，第四行用字符编码与序列等长的质量值。paired-end 数据通常分为 R1、R2 两个文件，记录顺序和 read 名必须保持对应。FASTQ 是历史形成的事实标准，早期 Sanger、Solexa 和 Illumina 变体曾使用不同的质量定义或 ASCII offset；处理旧档案时应根据元数据和软件文档确认编码，不能只凭字符外观猜测。[^fastq-format]
...
因此，校准良好的 Q20 碱基对应约 $10^{-2}$ 的错误概率，Q30 对应约 $10^{-3}$。Q 值是模型基于信号与校准数据给出的概率估计，不是某个碱基“已经被证明正确”的百分数；整条 read 的平均 Q 也不能显示低质量碱基集中在何处。不同平台、basecaller 和序列语境的质量值还可能校准不足，重要结论需要通过已知参考、技术重复、共识或正交方法检验。[^phred-error-probability]

过滤前应先确认文件是否完整、压缩流能否读到末尾、记录是否为四行结构、序列与质量字符串是否等长、read 名是否唯一或符合预期、R1／R2 是否同步、样本 barcode 与样本表是否一致。校验和与文件大小检验传输完整性，FASTQ 结构检验文件语法，质量分布检验测序信号；三者回答不同问题。
...
| --- | --- | --- | --- |
| 运行与拆样 | 总产量、每 lane／tile 质量、barcode 平衡、未分配 reads、对照文库表现 | 上样密度、成像、低多样性、index 读取或拆样异常 | 某个生物样本已充分覆盖目标 |
| read 与碱基 | read 数、总碱基数、长度分布、逐位点／逐 read 质量、N 含量 | 信号衰减、截短、basecalling 或文库大小异常 | 高 Q read 一定来自目标物种或目标分子 |
| 序列组成 | 逐位点 A/C/G/T、GC 分布、$k$-mer、过度代表序列、接头含量 | 接头读穿、引物残留、低复杂度、扩增或选择偏倚、污染候选 | GC 偏离一定是污染；所有富集序列都应删除 |
| 文库复杂度 | 唯一序列比例、重复率、UMI 家族大小 | 起始分子不足、过度扩增或高丰度生物分子 | 相同序列必为 PCR duplicate，或高重复文库必然失败 |
| paired-end | 配对保留率、方向、重叠、插入长度分布、异常 mate | 文库片段化、接头读穿、样本或文件配对错误 | 两端均高质量就能唯一定位重复区 |
| 比对后可用性 | 比对率、多重比对、目标区比例、深度、覆盖广度、插入长度、链特异性 | 参考不匹配、污染、捕获效率、文库方向或结构异常 | 高比对率自动证明样本身份与生物学假设 |
| 长 read／原始信号 | read N50 与完整分布、逐 read Q、产量随时间、通道／ZMW 利用、长度—质量关系、模型版本 | 样品剪切、短片段过多、孔道或聚合酶利用、basecaller 差异 | 一个 N50 足以描述长读段集合；新模型必然改善所有位点 |

逐位点质量下降常见于循环测序末端，但是否需要修剪取决于下游算法对质量值的使用方式和低质量尾部的实际长度。GC 分布应与物种、目标区和建库选择比较：外显子捕获、扩增子和 RNA-seq 不会复制全基因组的 GC 分布。重复也须结合实验解释；扩增前 UMI 可以估计原始分子，扩增后仅凭起止坐标标记 duplicates 会把天然高丰度 RNA、扩增子或受限片段误判为 PCR 副本。

污染检查同样需要分层。接头、引物和载体属于技术序列，宿主、共生体和环境微生物可能是实验对象的一部分，样本间串扰、错误 barcode 和参考数据库误注释又会产生不同模式。可先用过度代表序列和分类抽样寻找候选，再结合阴性对照、样本来源、lane 分布与独立标记判断；一次分类器命中不足以证明真实污染，更不能在未核对前删除整类 read。

## 修剪、过滤与复杂度保留 { #trimming-filtering }

接头读穿发生在插入片段短于测序循环时，仪器越过 insert 读入接头。可靠修剪利用已知接头序列、paired-end 重叠或经过验证的自动检测，并报告每个样本中被修剪的 reads、碱基数和长度分布。接头序列若恰与真实低复杂度或保守序列相似，过短匹配会误剪；参数应包含最小重叠、允许错配、最短保留长度和 pair 处理规则。

质量修剪和质量过滤分别改变 read 边界与是否保留整条 read。过强的 3′ 修剪会缩短可唯一定位的序列，滑动窗口会让不同 read 获得不同有效长度，固定 Q 阈值也没有考虑质量值的校准差异。有些现代比对器和变异模型会直接利用逐碱基质量，此时保留质量信息可能比预先删去大量碱基更合适。应在原始数据上建立基线，按明确的下游目的处理，再比较保留率、长度、比对与目标指标，而不是把修剪当作每个流程必有的仪式。

低复杂度和重复 read 也不宜默认删除。低复杂度可能来自接头或测序失败，也可能是微卫星、poly(A) 尾、真实重复区或实验富集对象；duplicate 可能来自 PCR，也可能来自高深度、小靶区或高表达转录本。过滤规则要区分技术伪影与研究信号，并保留未经修改的原始 FASTQ、处理后的派生文件、工具版本、参数和每类去除原因。原文件不可就地覆盖。

## 覆盖度、文库偏倚与可检出性 { #coverage-bias-detectability }

对大小为 $G$ 的单倍体基因组，若 $N$ 条长度为 $L$ 的 read 都来自均匀随机取样，名义平均深度可近似写成 $C=NL/G$。真实数据违反其中多个条件：read 长度不同，片段并非均匀抽取，部分碱基质量过低，重复 read 被排除，参考含缺口和重复，多重比对 read 不能唯一放置。报告“30×”如果没有说明分母、过滤和参考版本，只是一个不完整的平均数。

覆盖至少包含三个维度。depth 是一个位置被多少有效碱基观察；breadth 是目标区域中达到某个最小深度的比例；uniformity 描述深度在目标区之间是否均一。均值相同的两个样本，可以一个覆盖广而平缓，另一个在少数位点极深、许多位点空缺。用于变异检出的 callable coverage 还要同时满足碱基质量、比对质量、链与位置支持等条件；Picard 等工具因此分别报告平均深度、达到不同深度的碱基比例，以及因 duplicates、base quality、mapping quality 和重叠 mates 被排除的碱基。[^coverage-metrics]

GC 含量会影响变性、扩增、cluster 形成、捕获和比对；片段长度选择会排斥过短或过长分子；PCR 优先扩增某些分子并降低复杂度；杂交捕获受探针设计、同源区域和外显子边界影响；参考中的重复、替代单倍型和缺口又改变可比对性。增加总 reads 只能缓解随机取样不足，不能自动填平系统性空缺。实验设计应先界定需要检测的事件大小、等位基因比例和目标区域，再据文库复杂度、平台误差和可用分子数估计深度，而不是从一个跨实验固定倍数倒推所有项目。

RNA-seq 的 read 数和“覆盖”还受到表达量、转录本长度、RNA 完整性、选择方法和异构体结构控制，不能按基因组均匀抽样解释；外显子组的分母应是设计靶区、实际可捕获区还是临床报告区，也必须明确。长 read 需要同时给出长度分布和有效分子数，因为相同总碱基量可能来自许多中等长度 read 或少数超长 read，两者对组装和相位的贡献不同。

## 特殊文库怎样改变读段证据 { #assay-specific-read-evidence }

测序平台只负责读取被送入的分子，实验选择决定读段可以回答什么问题。

- 全外显子组测序通过杂交捕获富集设计区，未捕获的调控区和结构变异断点通常不在主要观察范围内；on-target rate、靶区深度广度与捕获均一性比总 read 数更直接。外显子组后续变异解释进入[基因组组装、变异与比较分析](genome_analysis.md)。
- RAD-seq、GBS 和其他简化基因组策略在限制性位点附近取样。酶切位点变异、甲基化敏感性、片段大小选择和等位基因 dropout 都会改变可见位点集合；它们适合在既定物种和群体设计中获得分散标记，不能等同低深度全基因组测序。
- RNA-seq 的 poly(A) 选择、rRNA 去除、链特异建库、片段化和逆转录决定哪些 RNA 及其哪一段进入 read。CLIP-seq／iCLIP／PAR-CLIP 把交联、免疫沉淀与特定截断或突变信号叠加到读段上，RIP-seq 则缺少同样的共价交联边界；信号必须结合输入、抗体和实验对照解释。定量、剪接和单细胞分析见[转录组、单细胞与空间组学](transcriptomics.md)。
...
这些文库也解释了为什么通用 QC 告警不能机械套用。若实验预期在 read 起始处出现固定引物、随机引物偏好、转座酶序列或交联诱导突变，相应的碱基组成异常可能是协议留下的证据；但只有预期、对照和样本间一致性能够把它与失败区分开。

## 从质量报告到分析决策 { #qc-to-decision }

一份可审查的测序交付应把实验与计算指标连成同一条链。样品层记录材料来源、提取批次、核酸完整性、纯度和投入量；文库层记录构建方法、扩增循环、barcode／UMI 设计、片段分布与浓度；运行层记录平台、flow cell、试剂、cycle、basecaller 和拆样版本；文件层记录 read 数、总碱基、校验和、FASTQ 编码和配对关系；QC 层保存原始与处理后的报告、过滤统计、异常解释及最终纳入或排除决定。

质量阈值应由计划中的分析来定义。用于粗略物种鉴定的数据、用于低频体细胞变异的数据和用于单倍型组装的数据，即使来自同一平台，也需要不同的分子数、深度、长度与错误控制。发现异常后先定位它属于样品、文库、运行、拆样、碱基判读还是参考比对，再决定重拆样、重新 basecall、修剪、过滤、补测或重建文库。只把低质量 read 删除，可能让报告更整齐，却不能修复样本交换、低复杂度或捕获空缺。

最终应同时保存异常本身和处理理由。FastQC／MultiQC 的原始报告、阈值来源、处理前后 read 数、未通过样本及其去向，都属于结果来源信息。一个“通过 QC”的标签只表示数据满足了已声明用途的最低标准；它不能替代后续比对、组装、变异、表达或实验验证，也不能把未被测到的分子改写为不存在。
