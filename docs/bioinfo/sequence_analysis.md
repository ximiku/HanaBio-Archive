...
序列比对把两个或多个 DNA、RNA 或蛋白质序列排成可以逐列比较的形式。这个排列并非原始数据中天然存在的事实，而是在字符顺序、替换得分、空位代价和边界条件共同限定下提出的一种对应关系。它可以提示结构域、催化残基、调控元件和共同祖先，也可以暴露插入缺失、重复和重排；它本身却不能单独确定祖先状态、功能是否相同，或某段相似究竟来自共同祖先、趋同、低复杂度还是偶然匹配。

数据库搜索是在更大的候选集合中反复进行这种比较。实际工作通常不是“找到最像的一条序列”便结束，而是依次回答：查询对象是否正确，比较的是整条序列还是一个结构域，搜索空间怎样构成，命中的统计显著性和覆盖范围如何，候选序列在物种树、基因家族和实验注释中处于什么位置。序列相似性由此成为证据链的入口，而不是功能结论的终点。
...
**同源**（homology）描述共同祖先关系，是一个历史命题，不是百分数。若证据支持两条序列来自共同祖先，可以称它们同源；若证据不足，则只能说存在一定程度的相似。基因谱系中的复制和物种分化还会产生不同关系：由物种分化事件分开的同源基因称为直系同源基因（ortholog），由基因复制事件分开的同源基因称为旁系同源基因（paralog）。这两类关系由基因树相对于物种树的事件解释来定义，并不简单等同于“不同物种中的同功能基因”和“同一物种中的不同功能基因”。直系同源通常是功能转移的重要线索，但功能仍可能发生分化；古老复制产生的旁系同源也可以分布在不同物种中。[^fitch-homology]

另一些相似来自共同结构域而不是整条蛋白质的一一对应。结构域融合、片段缺失、可变剪接和低复杂度尾部都可能使一条查询同时命中多个家族。此时局部比对能够正确找出保守区，却不能据此把完整蛋白质都归为同一功能。判断关系时应先检查结构域架构、序列长度和覆盖是否相称，再结合系统发育、基因邻域、结构和实验资料。

编辑距离和最长公共子序列有助于理解字符串算法，但它们只是生物序列比对的特殊简化。Hamming 距离只计算等长、无空位字符串中不同字符的个数；通常所说的 Levenshtein 编辑距离允许替换、插入和删除。生物序列比对则为不同替换赋予不同得分，并对空位的开启和延伸分别计价。它优化的是指定计分模型下的目标函数，不等同于寻找最长公共子序列，更不自动还原真实进化史。动态规划的一般思想见[算法、统计与计算基础](computational_foundations.md)。
...
真实序列中的随机匹配会产生密集噪声。窗口长度越大、阈值越高，图形越稀疏而偏向强相似；阈值越宽松，弱信号和噪声都会增加。蛋白质点阵图还可用替换矩阵累计窗口得分，而不只标记完全相同的残基。点阵图给出的是形态线索，具体断点和同源关系仍要通过可重复的比对、统计检验与基因组背景确认。

## 替换得分与空位模型 { #scoring-models }

一个比对的原始得分通常由各列替换得分与空位得分相加而成。核酸的简单模型可给匹配正分、错配负分；若问题需要，还可区分转换与颠换、背景碱基频率或不同位置的替换率。早期蛋白质计分曾从遗传密码中单碱基变化的可达性、氨基酸理化性质和经验交换频率等角度设计权重，这些探索保留了替换模型的发展路径。现代常用矩阵多从已知同源序列统计对数优势：某一残基对在相关序列中出现得比按背景频率偶然配对更频繁时得到正分，反之得到负分。得分表达的是一个替换模型中的相对证据，不是两个残基“化学相似度”的唯一尺度。

PAM 与 BLOSUM 是两条经典矩阵谱系。PAM 矩阵从较近缘蛋白质的可接受点突变出发，并借助模型外推到更长的演化距离；PAM 编号越大，代表模型中的替换距离越长。BLOSUM 从保守蛋白质区块直接统计，先按给定一致性阈值聚类近似序列；BLOSUM 编号越低，聚类阈值越低，通常用于表示较为分化的关系。二者的编号方向相反，也不能机械转换成固定的物种距离。矩阵的选择会改变弱匹配的排序，应依据核酸或蛋白质对象、预期分化程度、局部或全局目标以及工具的统计标定来决定。[^blosum]
...
其中开启新空位通常比延长已有空位代价更大。这让一段连续缺失比许多彼此分散的单残基空位更容易被接受。不同软件也可能采用 \(g_{\mathrm{open}}+kg_{\mathrm{extend}}\) 的记账约定，所以复现实验时要记录软件定义而不只记录两个数值。仿射空位可由多状态动态规划高效求解，是 Gotoh 算法的重要贡献。[^gotoh]

计分模型会塑造最终排列。错配代价过重可能制造许多空位，空位开启过轻可能拆散本应连续的结构域，末端空位是否计分则会影响片段与全长序列的比较。对编码序列还要留意移码和密码子边界；对蛋白质结构域、信号肽或无序尾部，也可能需要分区比对，而不是用一套参数覆盖所有区域。

## 成对序列比对 { #pairwise-alignment }

成对比对把两个序列前缀的最优解组织成矩阵。矩阵中的每一步可对应两个字符相配、在第一条序列中放置空位，或在第二条序列中放置空位；递推填表后再沿满足最优得分的路径回溯，得到具体排列。若存在并列路径，同一最优得分可以对应多个比对。算法保证的是指定计分、空位与边界条件下的数学最优，而不是唯一的生物学真相。

| 比对方式 | 边界条件的核心含义 | 适合的问题 | 主要风险 |
| --- | --- | --- | --- |
| 全局比对 | 两条序列都从端到端进入目标函数 | 长度相近、整体同源的基因或蛋白质 | 非同源尾部和结构域差异会牵动整条比对 |
| 局部比对 | 允许从得分下降处重新开始，只保留最高分局部区 | 结构域、外显子、保守片段和数据库搜索 | 强局部命中不代表全长同源或功能相同 |
...
启发式搜索先找可能产生高分比对的种子，再只对种子附近做扩展和精化。它用一部分敏感性换取数量级上的速度提升，使大型数据库搜索成为日常操作。未命中不等于数据库中绝无远缘同源序列：种子长度、替换矩阵、空位、掩蔽、组成校正和报告阈值都可能影响候选能否进入精化阶段。

FASTA 是早期广泛使用的序列相似性搜索体系。经典流程先以短词（k-tuple）寻找对角线上的密集匹配，连接或重评分候选区，再对最有希望的区域进行局部比对。较小的词长通常增加候选和灵敏度，也增加计算量；实际效果还取决于序列类型、矩阵和程序版本。FASTA 家族至今仍具有方法和软件价值，不能只把它写成“较慢的 BLAST”。[^fasta]

经典 BLAST 先寻找达到阈值的词邻域命中，再沿命中向两侧延伸形成高分片段对（high-scoring segment pair, HSP）。1990 年的原始算法突出无空位 HSP；后续 gapped BLAST 把带空位的精化与更高效的种子策略纳入标准实现，PSI-BLAST 则进一步引入位置特异性 profile。[^blast-original][^gapped-blast] 当前 BLAST 程序仍遵循“候选种子—延伸—统计报告”的总体框架，但不同任务使用的种子、索引、过滤和统计模型并不完全相同。

### BLAST 程序方向 { #blast-programs }

程序名中的翻译方向必须按查询和数据库分别理解。蛋白质查询与翻译后的核酸数据库不是同一个方向；这也是旧资料中最容易写反的一组概念。
...
NCBI 还为不同相似程度和数据规模提供 megablast、discontiguous megablast 等变体。选择程序时应先确定生物学对象，再考虑预期分化、查询长度和数据库规模；程序名不能代替这些前提。各方向和当前参数定义以 NCBI BLAST 官方指南为准。[^ncbi-blast-programs]
...
其中 \(E\) 是在该搜索空间中，随机情况下预期出现得分至少如此高的命中数；\(S'\) 是按计分系统标定后的 bit score。参数 \(K\) 和 \(\lambda\) 取决于替换与空位模型。E-value 可以大于 1，也不是“该命中为假”或“同源命题为真”的概率。在近似 Poisson 条件下，至少出现一次这类随机命中的概率为 \(1-e^{-E}\)，只有当 \(E\) 很小时才与 \(E\) 数值接近。[^karlin-altschul][^ncbi-blast-faq]
...
百分一致性很高但只覆盖十几个残基的匹配，证据可能弱于一致性较低却覆盖完整保守结构域的匹配。反过来，极长核酸序列中的短而完全一致片段也可能具有重要诊断价值。阈值必须由任务、序列长度、背景组成和可容忍的假阴性／假阳性共同决定，不能用一个通用 E-value 或一致性百分比覆盖所有对象。

在两个物种间互换查询并寻找 reciprocal best hit，可以快速筛选可能的一对一直系同源候选，但“彼此排名第一”不是直系同源的定义。谱系特异复制、基因丢失、数据库缺失、不同进化速率和结构域融合都可能让真实关系不呈现互为第一，或让旁系同源误入候选。可靠判断仍需更完整的家族采样、基因树—物种树协调和必要的共线性证据。
...
一条查询只携带自身每个位点的残基。多条可信同源序列的比对则能显示哪些位置高度保守、哪些位置允许一组替换、哪些区域经常插入或缺失。**profile** 把这种列特异信息变成搜索模型，能够识别单条序列比较难以捕获的远缘成员。

位置特异性计分矩阵（PSSM）为比对中每一列和每种残基给出得分。PSI-BLAST 首轮相当于蛋白质 BLAST 搜索，随后从达到纳入阈值的命中构建 PSSM，用它进入下一轮，再以新纳入序列更新模型。少量错误命中一旦进入 profile，可能在后续迭代中吸引更多无关序列，形成 profile drift；因此每轮都要检查新增序列、结构域边界和分类分布，而不是机械运行到收敛。PHI-BLAST 要求命中同时包含用户给定的模式并在其周围形成局部相似，适合“已知模式加上下文”的检索，却不能由模式本身确定家族。DELTA-BLAST 先借助保守结构域数据库构建 PSSM，再搜索蛋白质数据库。[^ncbi-profile-blast]

profile 隐马尔可夫模型（profile HMM）把匹配、插入和删除表示为不同状态，并为各位置设置发射概率和状态转移概率。它由经过审查的多序列比对训练，可以计算序列与家族模型的匹配，而不是“把 HMM 等同于一次局部多序列比对”。模型可以按全局、局部或混合方式配置；搜索灵敏度取决于训练序列的代表性、比对质量、模型边界和统计标定，不存在适用于所有家族的固定最少序列数。HMMER 提供 profile 对序列库的搜索，也提供单序列 `phmmer` 和迭代式 `jackhmmer` 等路径。[^eddy-profile-hmm][^hmmer]

profile 的优势也带来更强的依赖性：训练集中的分类偏倚、错误拼接、不同结构域的混合和冗余序列会写入模型。构建家族模型前应去除明显残缺或非同源片段，检查结构域边界，控制近重复序列的权重，并用未参与训练的阳性与困难阴性序列评估。profile 命中仍是家族关系证据，功能亚型和底物特异性往往需要更精细的关键位点、谱系和实验资料。
...
1. **渐进比对**先估计成对距离，构建指导树，再按指导树逐步完成序列—序列、序列—profile 或 profile—profile 比对。Clustal 谱系是经典代表。早期步骤一旦放错空位，错误可能随 profile 固化；指导树服务于比对顺序，不应直接当作最终系统发育树。
...
4. **随机或群体搜索**曾用遗传算法等方式探索巨大的排列空间，SAGA 等工具保留了这一方法史。它们说明 MSA 可以被表述为复杂优化问题，但并非当前所有规模的默认选择。

ClustalW、Clustal Omega、ClustalX、MUSCLE、MAFFT、T-Coffee 和 ProbCons 不是一条简单的“新工具淘汰旧工具”序列。ClustalW 的序列加权和位置特异空位思想具有教学价值，ClustalX 提供交互检查界面，Clustal Omega 面向大量蛋白质序列扩展，MAFFT 与 MUSCLE 提供多种速度—精度策略，T-Coffee 强调一致性证据。选择时应依据序列类型、数量、长度差异、预期结构域关系和下游用途，并在困难数据上比较多种参数或方法。MAFFT 的策略与限制可参见其方法论文。[^mafft]
...
成对相似性还可汇成距离矩阵，或把序列作为节点、把超过阈值的相似关系作为边，再用于层次聚类、连通分量或更专门的家族聚类。聚类在比对结果之上又加入距离转换、阈值、连接规则和代表序列选择，因而不是“批量比对”的同义词。单连接容易被桥接序列串起远距离簇，过严阈值则可能拆开真正多样的家族；用于去冗余、构建 orthogroup 或选择 MSA 代表序列时，应分别说明目标和规则。

## RNA 的序列与结构协同 { #rna-homology-search }

许多结构 RNA 的功能依赖碱基配对。一个茎区中的 GC 变成 AU，一级序列发生两处变化，配对结构却仍可能保留；这种协同替换使单纯逐字符搜索漏掉远缘同源物。结构感知方法将序列保守与配对关系共同纳入模型，关注的不只是“哪个碱基在这一列”，还包括“两列是否以兼容方式共同变化”。
...
Sankoff 类方法把折叠与比对联合求解，说明这两个问题在原理上可以同时处理，但直接精确计算非常昂贵。Dynalign、Foldalign、LocARNA 等方法以不同近似、约束和预计算降低代价，适合的序列数和长度也不同。实际分析可先用序列或 profile 找候选，再以结构感知方法精化；结构补充序列证据，却不能替代实验结构探测、保守性背景和假阳性评估。

## 高相似序列与全基因组比对 { #genome-scale-alignment }

当目标是把 mRNA、组装片段或近缘序列快速定位到基因组时，索引数据库并优先寻找高相似种子的工具往往更合适。BLAT 最初针对高相似 DNA 序列以及蛋白质／翻译查询设计，采用内存索引提高重复查询速度；它在转录本到基因组定位等任务中很有历史和现实价值，但不应作为远缘同源搜索的通用替代。现代短读长和长读长定位还要处理测序误差、剪接、重复和成对约束，属于[基因组组装、变异与比较分析](genome_analysis.md)的完整工作流。[^blat]

全基因组比较还要面对重复序列、大片段插入缺失、倒位、易位、片段复制和组装缺口。一个实用框架是先寻找局部相似块，再把方向一致、次序相容的块组织成更长关系。UCSC 比较基因组流程中的 chain 文件把局部块串联为允许两侧含空位的链；net 再按目标基因组坐标把链分层，优先放置较高分关系，并记录可填入的次级、倒位或非共线关系。chain 主要表达成组局部比对的连接，net 主要表达这些候选在目标坐标上的层次选择，两者不能互换。[^ucsc-chain][^ucsc-net]

BLASTZ 及其后继 LASTZ 是这套哺乳动物基因组局部比对方法史的重要部分；MULTIZ 则利用一系列成对关系和指定的系统关系逐步构建多物种基因组比对。[^multiz] 参考基因组的选择会使坐标、缺口归属和可投影区域呈现不对称，但不存在“参考序列必须比查询更长或更复杂”的普遍条件。重复和片段复制还会造成一对多关系，最高分链也未必等于每个位点的唯一正交来源。

因此，全基因组轨道中的“已对齐”不表示每个碱基都已证实直系同源。坐标转换工具把一个组装上的区间经由既有 chain／net 关系投影到另一个组装；成功转换不保证调控功能、等位状态或注释在新背景中保持，失败也不必然表示该序列在生物学上缺失。重要位点应检查两侧共线性、重复注释、组装质量和物种特异证据。

## 方法选择与可重复报告 { #selection-reporting }

方法选择应从待验证的命题出发，而不是从软件名称出发。
...
一个可复现的搜索记录至少应包含查询序列的登录号与版本或校验摘要、数据库名称与发布版、检索日期、数据库的物种范围、程序与版本、替换矩阵、空位参数、掩蔽与组成校正、迭代纳入阈值，以及保留命中的规则。结果表除了 accession、E-value 和 bit score，还应保存查询／目标覆盖、比对长度、identity、positive、gap、物种、条目审定状态和结构域范围。数据库条目的版本、坐标与互操作规则见[生物数据、格式与数据库](data_databases.md)，由命中走向功能注释的证据层级见[功能注释、富集与功能预测](functional_annotation.md)。

最后还要保存阴性结果的解释空间：查询是否过短或含错误，适合的翻译方向是否使用，低复杂度是否被完全掩蔽，数据库是否包含目标类群，profile 是否覆盖所研究的亚家族，阈值是否为特定目的而设置。序列比对最有力量的地方，在于它把生物学问题转化为可检查的字符对应和统计证据；它最需要克制的地方，也正是不能把这种对应直接升级为共同功能、唯一进化事件或实验事实。
