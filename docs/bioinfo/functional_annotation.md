...
预测的验证要匹配所声称的功能。酶活假说需要纯化蛋白或适当体系中的底物转化、动力学和催化残基对照；结合功能需要亲和力、特异性和竞争实验；通路角色可用遗传扰动、互补、时间顺序及下游读出；定位需在合适细胞和表达水平下用成像、分级或邻近标记确认。敲除出现表型可以支持基因参与过程，但不足以证明预测的分子活性；过表达救援实验也要排除剂量和异位定位效应。
