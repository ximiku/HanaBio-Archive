...
细菌 rRNA 常作为含 16S、23S、5S rRNA 和部分 tRNA 的长操纵子前体产生。RNase III 等先切开成对结构，随后由 RNase E／G、RNase T、RNase PH 等不同内外切酶形成成熟末端；具体酶和先后顺序随物种、操纵子和装配状态变化。核糖体蛋白结合、rRNA 折叠和碱基修饰与修剪交错进行，装配失败的 rRNA 还会被 RNase R、PNPase 等质量控制路线清除，所以“先全部修饰，再粗切，最后细修”并非普遍顺序。[^bacterial-rna-processing]

前体 tRNA 的 5′ leader 通常由 RNase P 切除；其细菌 RNA 亚基在适当条件下具有催化活性，蛋白亚基帮助底物选择和细胞内反应。3′ trailer 可由 RNase Z 或多种外切酶处理，末端 CCA 则可能由基因直接编码，也可能由 CCA-adding enzyme 无模板添加。少数细菌 tRNA 含 group I intron，另有异常 tRNA 会被再次加尾并转入降解。成熟 tRNA 上的修饰集中于结构核心和 anticodon loop 等关键位置，但每种修饰对折叠、氨酰化或解码的贡献须逐项实验确定。[^trna-maturation]
...
### 切割与加尾定义多数 mRNA 的 3′ 端 { #mrna-three-prime-end }

在多数哺乳动物蛋白质编码转录本中，CPSF 组分识别 AAUAAA 或相关上游元件，CstF 等结合下游富 GU 序列，CPSF73 在组装后的复合体中切开 RNA。poly(A) polymerase 随后无模板加 A，PABPN1 结合新生尾并提高延伸过程性；输出后主要由胞质 PABP 接管。poly(A) 尾影响输出、翻译和稳定性，但终止密码子来自编码区本身，加尾不会一般性地“创造终止密码子”。[^mrna-polyadenylation]

同一前体常有多个 cleavage and polyadenylation sites。alternative polyadenylation（APA）若改变 3′ UTR，可增减 microRNA 或 RNA-binding protein 的调控位点；若选择内含子或 coding region 内的位点，还可能改变蛋白 C 端。位点使用由顺式序列、核心加工因子浓度、RNA-binding proteins、转录速度和染色质背景共同偏置，产生短转录本并不自动表示翻译效率一定升高。复制依赖型 metazoan histone mRNA 则由 stem-loop binding protein、U7 snRNP 和 CPSF73 等定义 3′ 端，通常不加 poly(A) 尾。[^alternative-polyadenylation]

## 剪接体以动态 RNA 催化中心连接外显子 { #spliceosomal-splicing }
...
trans-splicing 把来自不同前体的 exon 接到一起。锥体虫等 kinetoplastids 给多数 mRNA 接上带帽 spliced leader，并用相邻的 trans-splicing 与 polyadenylation 从 polycistronic RNA 中释放单个 mRNA；部分线虫及其他真核生物也使用 spliced-leader trans-splicing。细胞器 group II intron 还可由分散转录的 RNA 片段在折叠后完成 trans-splicing。它们共享跨分子成键结果，却不是一套可互换的酶系统。[^trans-splicing]

## 自剪接内含子保存 RNA 催化的两种化学路线 { #self-splicing-introns }

group I intron 折叠成催化结构后，以游离 guanosine 的 3′-OH 进攻 5′ splice site，再由上游 exon 的 3′-OH 连接下游 exon，释放线性 intron。group II intron 则通常由内部 branch-point adenosine 的 2′-OH 发起，形成与 spliceosomal intron 相似的 lariat；不少 group II intron 在细胞内还需要 maturase、RNA helicase 或其他蛋白帮助折叠，并可作为 retroelement 移动。所谓 self-splicing 指某些 RNA 在适当体外条件下可催化成键，不表示它们在细胞中总能脱离蛋白独立完成成熟。[^self-splicing]
...
## tRNA 成熟汇合多条跨域保守而装置多样的路线 { #trna-processing }

三域 pre-tRNA 通常都要形成精确 5′ 与 3′ acceptor stem、CCA 末端和折叠后的 anticodon loop。RNase P 在细菌、古菌和许多真核核内系统是 RNA 催化的 RNP，但植物细胞核和细胞器等系统可由 protein-only RNase P（PRORP）完成相同切割。3′ 端可由 tRNase Z 或外切酶形成，CCA-adding enzyme 则以成熟 tRNA 结构而非核酸模板连续选择 C、C、A。真核 cytosolic tRNA^His^ 的 G−1 可由 Thg1 逆向添加，细菌同一识别位常由基因编码；这些差异使相同氨酰化身份从不同前体产生。[^trna-maturation]
...
## RNA 编辑改变序列，化学修饰改变核苷酸状态 { #rna-editing-modification }
...
## RNA 周转和质量控制决定哪些加工产物得以积累 { #rna-quality-control }
...
- Deutscher MP. [Twenty years of bacterial RNases and RNA processing: how we've matured](https://pmc.ncbi.nlm.nih.gov/articles/PMC4371297/). *RNA*. 2015;21:597–600.
- Yip WS, Vincent NG, Baserga SJ. [Ribonucleoproteins in archaeal pre-rRNA processing and modification](https://pmc.ncbi.nlm.nih.gov/articles/PMC3608112/). *Archaea*. 2013;2013:614735.
- Galloway A, Cowling VH. [mRNA cap regulation in mammalian cell function and fate](https://pmc.ncbi.nlm.nih.gov/articles/PMC6414751/). *Biochimica et Biophysica Acta - Gene Regulatory Mechanisms*. 2019;1862:270–279.
- Liu J, Lu X, Zhang S, Yuan L, Sun Y. [Molecular insights into mRNA polyadenylation and deadenylation](https://pmc.ncbi.nlm.nih.gov/articles/PMC9570436/). *International Journal of Molecular Sciences*. 2022;23:10985.
- Tian B, Manley JL. [Alternative polyadenylation of mRNA precursors](https://pmc.ncbi.nlm.nih.gov/articles/PMC5483950/). *Nature Reviews Molecular Cell Biology*. 2017;18:18–30.
- Tholen J, Galej WP. [Structural studies of the spliceosome: bridging the gaps](https://pmc.ncbi.nlm.nih.gov/articles/PMC9762485/). *Current Opinion in Structural Biology*. 2022;77:102461.
- Phizicky EM, Hopper AK. [tRNA processing, modification, and subcellular dynamics: past, present, and future](https://pmc.ncbi.nlm.nih.gov/articles/PMC4371247/). *RNA*. 2015;21:483–485.
- Turi Z, Lacey M, Mistrik M, Moudry P. [Impaired ribosome biogenesis: mechanisms and relevance to cancer and aging](https://pmc.ncbi.nlm.nih.gov/articles/PMC6520011/). *Aging*. 2019;11:2512–2540.
- Nishikura K. [A-to-I editing of coding and non-coding RNAs by ADARs](https://pmc.ncbi.nlm.nih.gov/articles/PMC4824625/). *Nature Reviews Molecular Cell Biology*. 2016;17:83–96.
- Read LK, Lukeš J, Hashimi H. [Trypanosome RNA editing: the complexity of getting U in and taking U out](https://pmc.ncbi.nlm.nih.gov/articles/PMC4835692/). *WIREs RNA*. 2016;7:33–51.
- He PC, He C. [m⁶A RNA methylation: from mechanisms to therapeutic potential](https://pmc.ncbi.nlm.nih.gov/articles/PMC7849164/). *The EMBO Journal*. 2021;40:e105977.
- Bresson S, Tollervey D. [Surveillance-ready transcription: nuclear RNA decay as a default fate](https://pmc.ncbi.nlm.nih.gov/articles/PMC5881035/). *Open Biology*. 2018;8:170270.
- Monaghan L, Longman D, Cáceres JF. [Translation-coupled mRNA quality control mechanisms](https://pmc.ncbi.nlm.nih.gov/articles/PMC10548175/). *The EMBO Journal*. 2023;42:e114378.
- Wilson RC, Doudna JA. [Molecular mechanisms of RNA interference](https://pmc.ncbi.nlm.nih.gov/articles/PMC5895182/). *Annual Review of Biophysics*. 2013;42:217–239.
- Shang R, Lee S, Senavirathne G, Lai EC. [microRNAs in action: biogenesis, function and regulation](https://pmc.ncbi.nlm.nih.gov/articles/PMC11087887/). *Nature Reviews Genetics*. 2023;24:816–833.

[^bacterial-rna-processing]: 细菌 mRNA、rRNA、tRNA 的加工与质量控制及不同 RNase 组合见 Deutscher 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4371297/)；RNase E／J／Y 启动周转和谱系差异见[细菌 mRNA 降解综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4431577/)。
[^trna-maturation]: RNase P、tRNase Z、CCA 添加、tRNA intron 和跨域装置差异见 Phizicky 与 Hopper 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4371247/)及[RNase P 综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC3904588/)。
[^archaeal-rna-processing]: 古菌 pre-rRNA 的 BHB 切割、box C/D／H/ACA sRNP 与修饰见 Yip 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC3608112/)；tRNA／rRNA splicing endonuclease 的底物范围见[机制研究综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC7714728/)。
[^mrna-capping]: m⁷G cap 的形成、CBC 介导的加工／输出和 cap 状态差异见 Galloway 与 Cowling 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC6414751/)。
[^mrna-polyadenylation]: CPSF73 cleavage、poly(A) polymerase、PABPN1 与 deadenylation 的结构和机制见 Liu 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC9570436/)。
[^alternative-polyadenylation]: APA 对 3′ UTR 和 coding region 的不同后果及位点选择因素见 Tian 与 Manley 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC5483950/)；U7／SLBP 依赖的 histone mRNA 3′ end formation 见[CPSF73 综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC8364479/)。
[^spliceosome]: major spliceosome 的组装、RNA 重排、两次 transesterification 和催化中心结构见 Tholen 与 Galej 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC9762485/)及 Alberts 等的[教材章节](https://www.ncbi.nlm.nih.gov/books/NBK26887/)。
[^minor-spliceosome]: U12-type intron 与 U11／U12／U4atac／U6atac snRNP 的底物和结构边界见[minor spliceosome 综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC3584512/)。
[^trans-splicing]: spliced-leader trans-splicing 在 kinetoplastids、nematodes 等生物中的装置和产物见[跨物种综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC3795323/)。
[^self-splicing]: group I／II intron 的 nucleophile、lariat chemistry 及其与 spliceosome 的联系见 Alberts 等的[教材章节](https://www.ncbi.nlm.nih.gov/books/NBK26887/)。
[^rrna-processing]: 真核 pre-rRNA cleavage、snoRNP 修饰和 pre-ribosome assembly 见 Turi 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC6520011/)；核仁空间组织与当前页的分工见[细胞核页面](../cell/nucleus_chromatin.md#nucleolus-organization)。
[^adar-editing]: ADAR 对 double-stranded RNA 的 A→I editing、recoding 与 innate immune functions 见 Nishikura 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4824625/)及 Eisenberg 与 Levanon 的[综述](https://www.nature.com/articles/s41576-018-0006-1)。
[^organelle-rna-editing]: plant organelle C→U／U→C editing 及 PPR factors 见 Hao 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC8511385/)；kinetoplastid gRNA-guided U insertion／deletion 见 Read 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4835692/)。
[^rna-modifications]: METTL3–METTL14、m⁶A site context 与 reader-dependent outcomes 见 He 与 He 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC7849164/)。
[^rna-exosome]: nuclear exosome、MTR4／TRAMP 和异常 RNA surveillance 见 Bresson 与 Tollervey 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC5881035/)；cytoplasmic deadenylation、decapping、XRN1 与 exosome 路线见[mRNA 降解综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4615280/)。
[^translation-coupled-surveillance]: NMD、NGD 与 NSD 的触发线索、ribosome collision 和正常调控底物见 Monaghan 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC10548175/)。
[^mirna-biogenesis]: canonical／non-canonical animal miRNA processing 与 Argonaute loading 见 Shang 等的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC11087887/)。
[^rnai-mechanism]: Dicer、Argonaute、guide-strand selection、target cleavage 与谱系差异见 Wilson 与 Doudna 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC5895182/)；plant AGO／DCL／RDR 分工见[植物 RNAi 综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4790879/)。
[^pirna-biogenesis]: mammalian primary piRNA processing、PIWI loading 和并非普遍存在的 ping-pong amplification 见 Fu 与 Wang 的[综述](https://pmc.ncbi.nlm.nih.gov/articles/PMC4114582/)。
