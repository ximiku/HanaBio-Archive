...
顺式作用元件通常通过与自身物理相连的核酸分子发挥作用，例如启动子、operator、enhancer、RNA 上的结合位点和结构开关；反式作用因子则是可扩散的蛋白质或 RNA。二者相遇的概率还受拷贝数、染色质或拟核组织、亚细胞位置和时间限制。细菌、古菌和真核生物都使用激活、抑制、反馈与组合控制，只是它们把这些逻辑安放在不同的基因组结构和细胞区室中。
