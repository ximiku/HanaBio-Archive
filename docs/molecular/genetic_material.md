...
遗传信息必须同时满足两类要求：它要能以足够稳定的形式跨细胞世代保存，又要能被复制、重组、表达和选择。对细胞生命而言，承担序列遗传的物质是 DNA；对一部分病毒而言，基因组则由 RNA 构成。核酸的化学结构解释了这些分子怎样保存序列，[核酸与核苷酸](../biochem/nucleic_acid.md)已经展开其共价骨架、碱基配对与高阶构象，本页关注的是另一层问题：实验怎样把核酸确认为遗传物质，连续序列怎样被划分为基因，全部遗传材料又怎样组织成基因组。
...
## 遗传物质由一系列可排除替代解释的实验证据确立 { #genetic-material-evidence }
...
Fraenkel-Conrat 和 Singer 用不同株系的烟草花叶病毒 RNA 与外壳蛋白重组感染性颗粒，子代特征随 RNA 而不是外壳蛋白来源传递。这类实验补全了概念边界：遗传物质的共同属性是可复制的核苷酸序列，不是必须具有脱氧核糖。细胞基因组和许多病毒基因组是 DNA，另一些病毒和类病毒则用 RNA 保存序列信息。[^tmv-rna]
...
朊粒显示蛋白质构象可以自我模板化传播。异常构象促进同种蛋白进入相似折叠状态，并可造成感染性或可遗传表型；这种过程传递的是构象状态，不是把一条蛋白质的氨基酸序列复制到另一条新生肽链。Prusiner 提出“proteinaceous infectious particle”概念后，蛋白质构象遗传成为独立的信息层次，但没有提供“蛋白质序列流回核酸”的机制。原素材把朊粒列作中心法则中“蛋白质到蛋白质”的未知路线，混合了序列与构象两种信息，应当据此拆开。[^prion-information]
...
### 转录单位、开放阅读框和基因并非同义词 { #gene-architecture }
...
### 假基因标签描述来源与编码缺陷，不保证绝对静默 { #pseudogenes-gene-families }
...
病毒基因组可以是 DNA 或 RNA、单链或双链、线形或环状、单分子或分节段，并可采用正义、负义或双义编码。部分病毒使用非标准碱基、共价末端蛋白、重复末端或部分双链结构。基因组构型决定复制时需要哪些聚合酶、是否必须随病毒颗粒携带转录酶，以及怎样产生可供宿主核糖体读取的 mRNA；完整复制路线见后续[DNA复制与逆转录](dna_replication.md)和[转录与RNA生物合成](transcription.md)，病毒生活史则由[病毒与亚病毒因子](../micro/viruses.md)展开。[^viral-genome-diversity]
...
原素材把古菌膜脂、细胞壁和鞭毛等一般细胞特征放在基因组章节中；这些知识应归入[细菌与古菌](../micro/prokaryotes.md)，本页只保留与遗传材料组织直接相关的内容。类似地，质粒提取的密度梯度步骤属于[核酸提取、扩增与分子操作](../exptech/biochem_molecular/nucleic_acid.md)，不能用操作方法代替质粒的生物学定义。
...
## 参考基因组是坐标模型，不是物种的唯一序列 { #reference-pangenome }
...
结构基因组学、功能基因组学、转录组学和比较基因组学怎样从这些序列产生可检验结论，属于[基因组分析](../bioinfo/genome_analysis.md)和后续生信页面；本页保留的核心原则是：序列、组装、注释和功能证明是四个不同层级，任何一层都不能自动替代下一层。
