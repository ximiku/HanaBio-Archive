...
群落调查得到的是样地、物种及其多度、盖度或出现记录组成的矩阵。植被分类把相似样地归入可复查的离散单位，群落排序（community ordination）则把组成差异投影到少数连续轴上；前者回答“哪些样地可归为同一类型”，后者回答“样地和物种主要沿哪些方向变化”。两者可以使用同一批数据，却具有不同的表示目标、方法假设与验证方式。

本页从北欧、法瑞、苏联／俄国和英美传统进入中国植被分类的等级与命名，再比较直接／间接梯度分析及常见排序方法。样方、多度、盖度、频度、重要值和多样性指标见[群落组成、结构与多样性](community_ecology.md)，干扰、岛屿周转、演替与顶极理论见[群落动态与演替](community_dynamics.md)。
...
## 从样地记录到可复查的群落格局 { #classification-ordination-inference }
...
离散类型和连续梯度可以同时成立：管理与制图需要可命名的植被单位，物种对水分、海拔和土壤的响应又可能连续且不同步。分类提供共同描述语言，排序展示组成变化的主要方向；两者都必须回到野外样地和独立环境证据，才不会把算法输出误作自然界预先存在的唯一边界。
