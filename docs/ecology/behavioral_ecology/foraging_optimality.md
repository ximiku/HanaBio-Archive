...
这里的“最优化”是研究方法中的条件判断。模型先规定可选行为、动物掌握的信息、收益单位和限制条件，再求出该组假设下表现最好的规则。观察结果若偏离预测，研究者便回头检查食物价值是否估错、动物是否正在学习、风险是否遗漏，或选择集合是否被形态和生理限制。经典模型因结构简洁而适合学习，现代证据则不断补入营养、认知、个体状态与社会环境；二者共同构成取食生态的知识路径。[^macarthur-pianka]

[取食、防御与争斗行为](../../zoology/behavior/foraging_defense.md)已经描述从搜索、辨认、攻击到搬运和储藏的动作，也保留鸟类、节肢动物与社会性昆虫的形态—行为实例。本页集中推导这些动作背后的收益模型及检验条件；[行为生态学总览](index.md)中的适合度、状态依赖和证据层级则是本页共同的方法前提。
...
最大化净摄入**速率**和最大化能量**效率**会给出不同预测。快速追击可能在每分钟取得更多食物，却消耗大量能量；缓慢行走或伏击每分钟收益较低，单位支出得到的净收益却可能更高。旧材料把两者称为“高投入—高收益”和“低投入—低收益”策略，这一对照可以保留，但哪种在高、低食物密度下占优取决于搜索方式、基础代谢、失败率和剩余可用时间。繁殖期亲鸟可能重视向巢内输送能量的速率，食物匮乏季节的个体可能更重视自身生存，二者不必采用同一收益单位。

收益最终要连接适合度。多吃能量可能增加存活和产卵，也可能带来毒素、营养失衡、受伤或天敌风险。模型常用能量率作为近似，是因为它容易测量且在许多条件下具有预测力；它并非所有动物在所有时刻共享的唯一货币。比较模型时，应把“动物最大化什么”当作待检验假说，而不是写进取食定义。
...
$\lambda_i$ 是搜索期间第 $i$ 类食物的单位时间遭遇率。分母中的 1 表示一单位搜索时间，第二项是这段搜索时间所引出的处理时间。旧材料中未写完的 $E/T$ 正是这个比率；把总搜索期写为 $T_s$ 时，分子与分母同乘 $T_s$，约去后得到相同结果。
...
大山雀在实验中面对大小食物时，整体方向符合高收益食物增多后排除低收益食物的预测，选择转变却没有像最简模型那样瞬间完成。采样和学习是重要解释：动物需要从近期遭遇估计 $\lambda_i$，环境改变后旧估计仍会保留一段时间。这个结果体现了经典模型的价值——预测的偏差帮助定位信息获取过程，而不是使整个最优化方法失效。[^great-tit-diet]
...
泛食与专食也不能仅由搜索和处理时间对调来定义。专化可能来自口器、消化、解毒、共生微生物、季节物候或学习；一个全年广食的动物可以在数小时内集中处理同一种食物。食谱模型描述的是某个时空尺度上的接受规则，分类时必须说明记录窗口。
...
**食物转换**指捕食者对相对丰富的猎物产生不成比例的集中捕食。它可以由进入富含该猎物的微生境、学习特定处理技术或形成选择性注意引起。Murdoch 的经典实验把捕食比例与猎物相对丰度的线性期望比较，说明弱偏好且有学习机会时更容易出现转换；强烈先验偏好或缺少学习机会时，猎物丰度改变未必造成同样结果。转换由相对攻击率定义，胃内容物中某食物比例升高还可能只是它更容易捕到。[^prey-switching]

反复遇到一种隐蔽猎物后，捕食者对其外形或微生境的检出率提高，称为**搜寻像**。稀有猎物可能因捕食者尚未形成有效搜寻像而具有暂时优势；密度极高时，检出又可能受饱食、处理饱和、猎物聚集或捕食者转移限制。旧材料所列“低密度和极高密度捕食率都低于预测、中等密度较高”不是搜寻像必然产生的固定曲线，需要把发现、攻击、处理和饱食分别测量。[动物行为页的搜寻像段落](../../zoology/behavior/foraging_defense.md#search-image)保留了感觉与学习机制，本页关注它怎样改变食谱和猎物频率。

## 斑块质量与边际值定理 { #patches-and-marginal-value-theorem }

食物在空间上常呈斑块分布。果树、花丛、潮池、种子堆和猎物群都可形成局部高收益区，斑块之间则需要旅行。斑块可以具有不同动力学：有些在一次访问内近似不变，有些因容易取得的食物先被吃掉而逐渐耗竭，还有些在降雨、潮汐、开花或猎物移动下周期更新。短寿命或一次性访问并不自动意味着斑块质量恒定，研究者仍要测量访问期间的收益曲线。
...
定理预测的是收益阈值，不规定动物必须直接计算导数。动物可采用“连续若干秒没有再捕到食物便离开”的**放弃时间**规则，或在最近捕获后扩大搜索半径。旧材料所称“限制搜寻期”可以理解为捕获后在局部进行集中搜索，随后回到更广范围移动。放弃时间会随环境平均质量、动物饥饿、天气和经验改变，并非一个固定生理常数。

斑块中的**放弃密度**（giving-up density, GUD）是动物停止取食时剩余的食物密度。Brown 的模型把边际收获率与代谢成本、捕食风险和放弃其他活动的机会成本相比较；遮蔽少或天敌线索强的斑块可能在较高剩余密度时被放弃。GUD 既可反映斑块偏好，也受初始食物、可探测性、竞争者和实验容器影响，不能把一个数值直接翻译成“恐惧程度”。[^giving-up-density]

## 斑块取样、学习与返回时间 { #sampling-learning-and-return }

动物通常不知道环境参数的真实值。它通过近期捕获间隔、气味、同类活动和过去访问更新斑块质量。周期性访问多个地点可以发现资源更新，但取样本身消耗时间；当前地点稳定可靠时，频繁检查其他地点未必有利。旧材料中“无论当前斑块能否满足需求都周期取样”应改为条件性预测：环境变化越快、地点间差异越大、旧信息失效越快，重新取样的价值通常越高。

食物能在离开后再生或补充时，动物还要决定何时返回。过早返回只遇到尚未恢复的斑块，过晚则可能被竞争者先取走。独占领域使更新率更可预测，却也需要巡护成本；多个个体共享资源时，返回时间受他者访问影响，稳定的个人路线可能被打乱。花蜜、潮间带猎物和果实的补给曲线不同，不能用一个固定巡回周期概括。

经验能够把多个地点连接成**陷阱线式路线**。熊蜂会反复访问一组花，并逐渐缩短路线；实验显示，它们在路线总长度与优先访问高报偿地点之间权衡，并不只是每一步都飞向最近花朵。对道路、河岸或树冠层中的动物，风、坡度、遮蔽和地标可靠性也会使能量或风险最小路线偏离几何最短线。[^bumblebee-trapline]
...
许多动物必须把食物送回巢穴、洞穴、幼体或储藏点，称为**中心地点取食**。一次往返包括外出旅行、斑块内搜索、逐个收集、负载返回和卸载。随着口中或身体上的猎物增多，继续收集常越来越慢；较长旅行又使空载往返更不划算。因此远处取食往往对应较大的运量或较长斑块停留，但负载降低飞行速度、增加被盗或使食物腐败时，关系会改变。

旧材料“琼鸟食物运量”对应经典椋鸟育雏研究。亲鸟在取食地逐个收集无脊椎动物，口中已有猎物越多，继续发现和夹持下一只通常越困难；到巢与取食地之间的旅行越久，平均每次带回的猎物数往往越多。椋鸟实验同时提醒，距离—运量关系并不能只归因于一条递减收获曲线，负载运输成本、幼鸟摄食瓶颈、巢址风险和其他活动时间都可能参与。[^starling-central-place]
...
平均食物量相同的两个斑块可以具有不同变异：一个每次提供稳定的小份食物，另一个可能得到大份，也可能完全落空。**风险敏感取食**讨论动物对这种产出分布的选择。简单的能量预算规则预测，若稳定方案足以越过当日生存阈值，降低方差更有利；若稳定方案几乎必然低于阈值，接受高方差才保留达到阈值的机会。由此产生的“接近饥饿时偏向风险”是特定收益函数下的预测，饥饿同时损害运动或高方差收益仍不足以救急时，动物仍可能选择稳定资源。早期墨西哥灯草鹀（*Junco phaeonotus*）实验提供了风险敏感偏好的实证起点，后续工作则显示状态、时间范围和学习会改变方向。[^risk-sensitive]
...
松鼠把较大食物搬到树上、把小食物就地处理，是处理收益与暴露风险的直观例子。搬运大食物的时间可由更安全地点的长处理阶段抵消；小食物很快吃完，搬运成本反而不值得。实验需要同时操纵食物大小、遮蔽和天敌线索，才能确认是安全收益而非食物可携带性造成差异。

竞争同样改变风险权衡。干扰竞争可能缩短处理时间、扩大食谱或迫使弱势个体进入边缘斑块；资源被他者消耗则降低等待价值。旧材料所说“竞争越强、捕食越弱、猎物越重要，动物对安全要求越低”可保留为方向性假说，但三项因素会交互，优势等级、可替代食物和逃逸能力还会改变结果。所谓小蜘蛛去网边缘、大蜘蛛留在网中心的例子，应先确认网的所有者、食物分布、配偶机会和捕食风险，不能只按体形赋予单一动机。

## 社会性昆虫的群体取食 { #social-insect-foraging }

社会性昆虫把个体采集决策与群体招募连接起来。单只工虫使用局部信息，食物搬回速度、舞蹈、踪迹信息素和巢内接触又改变下一批外出者的方向。群体层面的资源分配由许多反馈形成，不需要每只个体掌握整片环境，也不能假定群体总能达到全局最优。

### 蜂类的专访与招募 { #bee-specialization-and-recruitment }

开花植物常分散成短暂斑块，每朵花提供的报偿有限。许多独居蜂在物种或个体层面专访有限植物类群；社会性蜂的整个群体可同时利用多种花源，而单个采集者在一次路线中保持花常性。这是旧材料“独居蜂较专、群居蜂群体广而个体专”的可用比较，但专化范围由口器、花期、学习和当地植物群落决定，不是独居与群居的固定二分。

熊蜂缺少蜜蜂摆尾舞那样的远距离方向—距离编码，却能以花源气味、回巢活动和社会线索提高外出概率，也能形成个人陷阱线路，因此“没有良好通讯方式”并不准确。蜜蜂侦察蜂则把花源方向、距离和收益带回蜂巢；舞蹈强度与持续时间使高报偿地点获得较多招募，食源枯竭后舞蹈衰减。群体分配还受糖浓度、飞行成本、天气和巢内需求影响，并非所有个体只访问瞬时能量率最高的花源。[^honeybee-allocation]

### 行军蚁的集体扫荡 { #army-ant-raids }

行军蚁的典型取食本身就是集体突袭，而不是先有彼此独立捕食、只有面对大猎物才临时合作。不同支系可形成较窄的纵队袭击或宽阔的群集突袭，猎物以其他社会性昆虫和林下无脊椎动物为主；猎物大小、巢群规模、地形与交通拥堵共同影响队形。工蚁通过接触、化学踪迹和局部交通反馈集中到猎物丰富方向，沿路线形成的临时猎物堆还可提高回收效率。旧材料关于“小群利用聚集食物、大群利用小而活动猎物”的对照可作为比较问题保留，不能作为所有行军蚁队形的统一因果律。[^army-ant-raids]

### 收割蚁的主干道与支路 { #harvester-ant-trails }

收割蚁在干旱和季节性地区采集种子，巢口向外可形成长期使用的主干道，食物新出现时再建立较短暂支路。踪迹缩短重复探索，也会把工蚁集中到有限通道；邻巢竞争、植物斑块和多个巢口共同塑造道路方向。以欧洲 *Messor* 收割蚁为例，主干道既反映中心地点运输效率，也受巢内和巢间竞争影响，不能只解释为永久划分互不重叠的觅食地。[^harvester-ant-trails]

旧材料还比较“个体觅食”和“纵队扫荡”，并把它们分别连接集中丰富与分散稀少资源。真实对应关系依物种和招募系统而变：集中高价值食物常值得强招募，广泛散落的小种子也可能由许多个体沿主干道独立搜索。保留红、黑、褐农蚁等传统类型名称时，应回到具体种、巢结构和资源图确认，不以中文俗名直接推出食性完全不同或竞争已经消失。

### 白蚁的低质量资源与真菌圃 { #termite-foraging-and-fungus-gardens }

白蚁利用木材、枯落物、土壤有机质和草本等低氮、难分解资源，食物并非无限，修路、筑巢、防御和维持微环境也都消耗材料与能量。白蚁与肠道细菌、古菌和原生生物的共生提高木质纤维利用；只有大白蚁亚科（Macrotermitinae）培养 *Termitomyces* 真菌，工蚁品级的个体把采集的植物材料加工成菌圃，真菌与肠道过程共同分解植物生物质。菌圃和巢内代谢确会产热，却不能据此把真菌圃的功能归结为“给巢加热”。[^fungus-growing-termites]

伤残个体、尸体、排泄物与巢材的再利用可使营养回到群体循环，减少低质量环境中的损失；同类相食也受感染风险、伤势程度和群体需求调节。这些过程体现资源回收，而不是“觅食和筑巢不消耗能量”。旧材料的节能思想由此得到保留，同时被放进真实的资源与工程成本中。

## 储食的当前成本与未来价值 { #food-caching }

储食把食物的获得与消费分离。动物在丰盛期收集食物，选择集中储藏或分散储藏，稍后再依靠空间线索、气味或系统搜索恢复。它常见于资源季节性强或短期波动大的环境，也存在于气候温和但资源脉冲明显、竞争激烈或繁殖期需求集中的地区，因此“主要发生在严酷地区”只能视为环境倾向。
...
储食学习随个体发育成熟。复杂空间记忆可能使幼体独立觅食较晚，但发育速度还受亲代喂养、运动结构、机会和社会学习影响。鸦科的“什么—哪里—何时”记忆与未来需求实验说明部分物种能灵活利用时间信息；完整认知证据与动作过程保留在[动物行为页](../../zoology/behavior/foraging_defense.md#food-caching)，本页只把记忆能力作为未来收益能否实现的约束。

## 从模型预测到生态后果 { #testing-and-ecological-consequences }

检验取食模型需要先记录动物实际遇到什么。食谱资料只显示吃了什么，没有提供被拒绝的食物；斑块停留时间没有同时测量旅行时间和收益曲线，也不能检验边际值定理。较完整的研究会记录遭遇、接受、失败、处理、移动、警戒和摄入，并测量食物能量、营养、可探测性、更新率和竞争者密度。
...
动物的选择会反馈到资源分布。集中捕食常见猎物可以产生频率依赖死亡，斑块离开规则改变资源被耗竭的程度，储食把种子搬到新地点，花访路线影响植物授粉连接，蚁路又把营养和种子沿固定通道再分配。旧材料把这些结果概括为“资源再分”，这个概念值得保留；具体方向则由资源更新、消费者数量、空间尺度和未取回储藏共同决定。种群与群落后果可继续参照[捕食、寄生与互利](../consumer_resource_interactions.md)，空间分布与理想自由分布则由下一页[栖息地、领域与捕食关系](habitat_predation.md)展开。
