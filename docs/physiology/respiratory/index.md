...
!!! note "相同分钟通气量可以产生不同肺泡通气量"
    ...
    若仅作算例并假定 $V_D=150\ \mathrm{mL}$，$V_T=500\ \mathrm{mL}$、$f=12\ \mathrm{min^{-1}}$ 时，$\dot V_E=6.0\ \mathrm{L/min}$，而 $\dot V_A=4.2\ \mathrm{L/min}$。若改为 $V_T=250\ \mathrm{mL}$、$f=24\ \mathrm{min^{-1}}$，分钟通气仍是 $6.0\ \mathrm{L/min}$，肺泡通气却降为 $2.4\ \mathrm{L/min}$。在分钟通气相同时，浅快呼吸使更大比例的通气进入无效腔，从而降低肺泡通气量。

## 用力呼气的动态气道压缩 { #forced-expiration }
