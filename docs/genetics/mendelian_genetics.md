...
经典模型把遗传信息视为可分离的颗粒，同时允许基因在物理上继续细分。基因内部可以发生突变和重组，同一基因也可以有许多等位变异；在一次特定杂交中，只要这些变异能够作为座位上的状态被追踪，传递遗传学仍可把它们当作分析单位。现代遗传学因而保留孟德尔的实验逻辑，同时用染色体、DNA 序列和分子通路解释这些单位为何分离、何时不独立，以及为何同一基因型未必产生同一表型。[^mendel-chromosome]
...
棋盘格可以枚举少数座位的配子组合；遗传模型还要说明每类配子的概率、受精组合的概率，以及哪些基因型被合并成同一表型。这种概率表达也更容易扩展到多座位、复等位基因和不完全外显的情形。
...
在细胞层面，同源染色体于减数第一次分裂分向两极，使同一座位的两个等位基因进入不同的单倍体产物；受精再恢复成对状态。这个解释以规则减数分裂、配子随机结合、各基因型在观察前没有差异性死亡或取样偏倚为前提。染色体不分离、减数分裂驱动、配子竞争、合子致死和分型误差都可能改变实测计数，需要用独立证据区分分离过程与观察偏差。[^segregation-boundaries]

测交把这种分离转成直接读出。若显性表型个体是 $AA$，与 $aa$ 交配的子代全为 $Aa$；若它是 $Aa$，子代预期为 $Aa:aa=1:1$。同一设计用于两个座位时还可揭示连锁，重组分数和遗传图距见[连锁、交换与遗传作图](linkage_mapping.md)；“测交”描述交配设计，可以服务于多种遗传问题。
...
人类家系还会受到新生变异、生殖系或体细胞嵌合、年龄相关外显、误诊、收养关系和小家系的影响。相似疾病可有遗传异质性，同一变异也可呈多效性和可变表现度。HbS、地中海贫血、G6PD 缺乏和凝血因子缺陷分别展示了蛋白聚合、亚基供给、氧化防线和止血通路的分子后果，说明“等位传递”与“表型形成”需要分层分析；相应机制见[蛋白质功能](../biochem/protein_function.md#hbs-polymerization)、[磷酸戊糖途径](../biochem/ppp.md#g6pd-deficiency)和[生理性止血](../physiology/blood/blood_hemostasis.md)。家系判读、变异证据、遗传检测和伦理边界见[人类遗传学](developmental_human.md)。[^human-genetic-evidence]
